...
void main() {
  ...
  final SerializableFinder createWalletScreen =
      find.text('CREATE A WALLET'); //  - 4 Verify Seed Phrase Screen
  final SerializableFinder createWalletNameScreen =
      find.text('WELCOME'); //  - 5 Create Password Screen
  final SerializableFinder newSeedPhraseScreen =
      find.text('New Account'); //  - 6 Accept Disclaimer Screen
  final SerializableFinder checkSeedPhraseScreen =
      find.text('Check seed phrase'); //  - 7 Create PIN Code Screen
  final SerializableFinder createPasswordScreen =
      find.text('CREATE A PASSWORD'); //  - 8 Main Portfolio Screen
  final SerializableFinder disclaimerScreen =
      find.text('Disclaimer & ToS'); //  - 9 Settings Screen (Delete Wallet)
  ...
  final SerializableFinder createWalletBtn =
      find.byValueKey('createWalletButton'); //
  ...
  final SerializableFinder newAccountScrollable =
      find.byValueKey('new-account-scrollable'); //
  ...
  final SerializableFinder goBackAndCheckSeedAgain =
      find.text('GO BACK AND CHECK AGAIN'); //
  final SerializableFinder whichWordField =
      find.byValueKey('which-word-field'); //
  ...
  group('| (Scenario 1) Create new wallet   ', () {
    ...
    test('| -1.2- | Check back-btn from wallet-naming-screen (works)',
        () async {
      expect(await driver.getText(createWalletNameScreen), 'WELCOME');
    ...
    test(
        '| -1.6- | Check wallet name $testNewWalletName still on naming-wallet-screen after going back from save-seed-screen.',
        () async {
      expect(await driver.getText(createWalletNameScreen), 'WELCOME');
    ...
    test('| -1.7- | Check back-btn from wallet-naming-screen works (2nd time)',
        () async {
      expect(await driver.getText(createWalletNameScreen), 'WELCOME');
    ...
    test(
        '| -1.8- | Check wallet with name $testNewWalletName was NOT created after comeback from save-seed-screen',
        () async {
      expect(await driver.getText(createWalletScreen), 'CREATE A WALLET');
      ...
      if (present) {
        print(
            'ALARM! ALARM! wallet is created after comeback from save-seed-screen. ALARM ALARM!');
    ...
    test(
        '| -1.10- | Check you CAN NOT create a wallet without a name i.e. after tapping on'
        ' grey-setup-btn with empty wallet-name-field, we should stay on the same page.',
        () async {
      expect(await driver.getText(createWalletNameScreen), 'WELCOME');
    ...
    test(
        '| -1.11- | Check you can create a wallet with name = $testNewWalletName',
        () async {
      expect(await driver.getText(createWalletNameScreen), 'WELCOME');
    ...
    test(
        '| -1.12- | Check seed refresh works - test would fail if initialSeed == postRefreshSeed',
        () async {
      expect(await driver.getText(newSeedPhraseScreen), 'New Account');
    ...
    test('| -1.13- | Check if seed copy/paste works', () async {
      ...
      await driver.scrollUntilVisible(newAccountScrollable, next,
          dyScroll: -300);
      ...
      await driver.scroll(
          whichWordField, 0, 0, const Duration(milliseconds: 1100));
    ...
    test('| -1.16- | Verify seed', () async {
      ...
      for (int i = 0; i <= 2; i++) {
        ...
        await driver.getText(whichWordText).then((val) {
          whichWord = int.tryParse(
                  val.split(' ')[3].replaceAll(RegExp(r'[^\w\s]+'), '')) -
              1;
        });
        await driver.waitFor(whichWordField);
        await driver.tap(whichWordField);
