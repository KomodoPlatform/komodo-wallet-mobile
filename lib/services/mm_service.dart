...
class MMService {
  ...
  Future<void> updateMmBinary() async {
    ...
    if (md5h == lastHash) {
      Log('mm_service:181', 'MM matches the assets/ hash, skipping update');
      await prefs.setInt('mm2.lastCheck', ms);
      return;
    }

    Log('mm_service:186', 'Updating MM…');
      ...
      try {
        ...
        Timer.periodic(const Duration(seconds: 2), (_) {
          ...
          checkStatusmm2().then((int onValue) {
            Log('mm_service:367', 'mm2_main_status: $onValue');
            if (onValue == 3) {
              _running = true;
              _.cancel();
              initCoinsAndLoad();
  ...
  List<_UpdReason> _lookupReasons(String chunk) {
    ...
    for (RegExpMatch mat in sending.allMatches(chunk)) {
      //Log('mm_service:432', 'uuid: ${mat.group(1)}; sample: ${mat.group(0)}');
    ...
    for (RegExpMatch mat in dashboard.allMatches(chunk)) {
      //Log('mm_service:440', 'uuid: ${mat.group(1)}; sample: ${mat.group(0)}');
      reasons.add(_UpdReason(sample: mat[0], uuid: mat[1]));
    }
  ...
  Future<void> initCoinsAndLoad() async {
    try {
      ...
      Log('mm_service:480', 'All coins activated');
      await coinsBloc.loadCoin();
      Log('mm_service:482', 'loadCoin finished');
    } catch (e) {
      print(e);
    }
  }
  ...
  Future<List<Balance>> getAllBalances(bool forceUpdate) async {
    Log('mm_service:530', 'getAllBalances');
