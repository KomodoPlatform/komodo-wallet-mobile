...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m73(seconde) => "訂單匹配中, 請稍等 ${seconde} 秒!";

  static m1(index) => "請輸入第 ${index}. 個字";

  static m2(index) => "您助記詞的第 ${index}. 個字是什麼？";
  ...
  static m41(coinName, number) => "最低可賣出數量 ${number} ${coinName}";

  static m42(coinName, number) => "最低可買入數量 ${number} ${coinName}";

  static m17(coinName) => "當前無 ${coinName} 匹配訂單 - 請稍後再試, 或建立訂單。";

  static m18(assets) => "${assets} 資產";

  static m19(amount) => "點擊查看${amount}訂單";

  static m20(coinName, address) => "我的 ${coinName} 地址: \\n${address}";

  static m21(appName) => "ATOMIC DEX 是次世代多幣別錢包, 具有原生第三代的DEX功能等等。";

  static m22(amount, coinName) => "提領${amount} ${coinName}";

  static m23(amount, coin) => "您將收到${amount} ${coin}";
