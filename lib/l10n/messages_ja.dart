...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m0(name) => "${name} の有効化に成功しました!";

  static m1(title) => "${title} 住所の連絡先のみを表示";

  static m2(abbr) => "${abbr} が有効化されていないため、${abbr} アドレスに送金できません。ポートフォリオへどうぞ。";

  static m3(appName) => "いいえ！ ${appName} は非親権者です。秘密鍵、シード フレーズ、PIN などの機密データを保存することはありません。このデータはユーザーのデバイスにのみ保存され、デバイスから離れることはありません。あなたは自分の資産を完全に管理しています。";

  static m4(appName) => "${appName} は、モバイルでは Android と iPhone の両方で利用でき、デスクトップでは Windows、Mac、および Linux オペレーティング システムで利用できます。";

  static m5(appName) => "他の DEX では通常、単一のブロックチェーン ネットワークに基づく資産の取引のみが許可され、プロキシ トークンが使用され、同じ資金で単一の注文のみが許可されます。 ${appName} を使用すると、プロキシ トークンを使用せずに、2 つの異なるブロックチェーン ネットワーク間でネイティブに取引できます。同じ資金で複数の注文を出すこともできます。たとえば、KMD、QTUM、または VRSC で 0.1 BTC を販売できます。最初に約定した注文は、他のすべての注文を自動的にキャンセルします。";

  static m6(appName) => "各スワップの処理時間は、いくつかの要因によって決まります。取引された資産のブロック時間は、各ネットワークによって異なります (通常、ビットコインが最も遅いです)。さらに、ユーザーはセキュリティ設定をカスタマイズできます。たとえば、${appName} に、わずか 3 回の確認後に KMD トランザクションを最終と見なすように依頼できます。これにより、<a href=\"https://komodoplatform.com/security-delayed-proof- of-work-dpow/\">公証</a>。";

  static m7(appName) => "${appName} で取引する際に考慮すべき 2 つの手数料カテゴリがあります。 1. ${appName} は、テイカー オーダーの取引手数料として約 0.13% (取引量の 1/777、ただし 0.0001 以上) を請求し、メイカー オーダーの手数料はゼロです。 2. メーカーとテイカーの両方が、アトミック スワップ トランザクションを行う際に、関連するブロックチェーンに通常のネットワーク料金を支払う必要があります。ネットワーク手数料は、選択した取引ペアによって大きく異なります。";

  static m8(name, link, appName, appCompanyShort) => "はい！ ${appName} は、<a href=\"${link}\">${appCompanyShort} ${name}</a> を通じてサポートを提供しています。チームとコミュニティはいつでも喜んでお手伝いします!";

  static m9(appName) => "いいえ！ ${appName} は完全に分散化されています。第三者によるユーザー アクセスを制限することはできません。";

  static m10(appName, appCompanyShort) => "${appName} は ${appCompanyShort} チームによって開発されました。 ${appCompanyShort} は、アトミック スワップ、Delayed Proof of Work、相互運用可能なマルチチェーン アーキテクチャなどの革新的なソリューションに取り組んでいる、最も確立されたブロックチェーン プロジェクトの 1 つです。";

  static m11(appName) => "絶対！詳細については、<a href=\"https://developers.atomicdex.io/\">開発者向けドキュメント</a>を参照するか、パートナーシップについてお問い合わせください。特定の技術的な質問がありますか? ${appName} 開発者コミュニティはいつでもお手伝いします!";

  static m12(batteryLevelCritical) => "交換を安全に行うには、バッテリーの充電が重要です (${batteryLevelCritical}%)。充電してからもう一度お試しください。";

  static m13(batteryLevelLow) => "バッテリーの充電が ${batteryLevelLow}% 未満です。電話の充電を検討してください。";

  static m14(seconde) => "オーダーマッチが進行中です。${seconde} 秒お待ちください!";

  static m15(index) => "${index}単語を入力してください";

  static m16(index) => "シード フレーズの ${index} 単語は何ですか?";

  static m17(name) => "連絡先 ${name} を削除してもよろしいですか?";

  static m18(iUnderstand) => "カスタム シード フレーズは、生成された BIP39 準拠のシード フレーズまたは秘密鍵 (WIF) よりも安全性が低く、クラックされやすい可能性があります。リスクを理解し、自分が何をしているかを理解していることを確認するには、下のボックスに「${iUnderstand}」と入力してください。";

  static m19(abbr) => "入力 ${abbr} アドレス";

  static m20(gas) => "ガスが不足しています - 少なくとも ${gas} Gwei を使用してください";
  ...
  static m35(coin) => "${coin} フォーセットにリクエストを送信しています...";
  ...
  static m39(abbr) => "${abbr} 取引手数料を支払うのに十分な残高がありません";

  static m40(coinAbbr) => "${coinAbbr} は利用できません:(";

  static m41(coinName, number) => "最低販売額は ${number} ${coinName} です";

  static m42(coinName, number) => "最低購入金額は ${number} ${coinName} です";

  static m43(buyCoin, buyAmount, sellCoin, sellAmount) => "注文の最小額は ${buyAmount} ${buyCoin} (${sellAmount} ${sellCoin}) です";

  static m44(coinName, number) => "最低販売額は ${number} ${coinName} です";

  static m45(minValue, coin) => "${minValue} ${coin} より大きくなければなりません";

  static m46(appName) => "現在、携帯データ ネットワークを使用しており、${appName} P2P ネットワークに参加すると、インターネット トラフィックが消費されることに注意してください。携帯電話のデータ プランが高額な場合は、WiFi ネットワークを使用することをお勧めします。";

  static m47(coin) => "最初に ${coin} を有効にして、残高をチャージしてください";

  static m48(number) => "${number} オーダーを作成:";

  static m49(coin) => "${coin} の残高が少なすぎます";

  static m50(coin, fee) => "料金を支払うのに十分な ${coin} がありません。 MIN 残高は ${fee} ${coin} です";

  static m51(coinName) => "${coinName} の金額を入力してください。";

  static m52(coin) => "取引に十分な ${coin} がありません!";

  static m53(sell, buy) => "${sell}/${buy} スワップが正常に完了しました";

  static m54(sell, buy) => "${sell}/${buy} スワップに失敗しました";

  static m55(sell, buy) => "${sell}/${buy} スワップ開始";

  static m56(sell, buy) => "${sell}/${buy} スワップがタイムアウトしました";

  static m57(coin) => "${coin} のトランザクションを受け取りました!";

  static m58(assets) => "${assets} アセット";

  static m59(coin) => "${coin} の注文はすべてキャンセルされます。";

  static m60(delta) => "便宜: CEX +${delta}%";

  static m61(delta) => "高価: CEX ${delta}%";
  ...
  static m67(appName) => "${appName} を使用できるデバイスはどれですか?";

  static m68(appName) => "${appName} での取引は、他の DEX での取引とどう違うのですか?";

  static m69(appName) => "${appName} の料金はどのように計算されますか?";

  static m70(appName) => "${appName} の背後にいるのは誰ですか?";
  ...
  static m79(maxCoins) => "アクティブなコインの最大数は ${maxCoins} です。いくつか無効にしてください。";

  static m80(coin) => "${coin} はアクティブではありません!";

  static m81(coin) => "売却する${coin}の金額を入力してください";

  static m82(coin) => "${coin} をアクティベートできません";

  static m83(description) => "mp3またはwavファイルを選択してください。 ${description} になったら再生します。";

  static m84(description) => "${description} の場合にプレイ";

  static m85(appName) => "ご不明な点がある場合、または ${appName} アプリで技術的な問題を発見したと思われる場合は、報告してチームからサポートを受けることができます。";

  static m86(coin) => "最初に ${coin} を有効にして残高をチャージしてください";
  ...
  static m91(version) => "バージョン ${version} を使用しています";

  static m92(version) => "バージョン ${version} が利用可能です。更新してください。";

  static m93(appName) => "${appName} の更新";

  static m94(coinAbbr) => "${coinAbbr} をアクティベートできませんでした";

  static m95(coinAbbr) => "${coinAbbr} をアクティベートできませんでした。アプリを再起動してもう一度お試しください。";

  static m96(appName) => "${appName} モバイルは、ネイティブの第 3 世代 DEX 機能などを備えた次世代のマルチコイン ウォレットです。";

  static m97(appName) => "以前にカメラへの ${appName} アクセスを拒否しました。 QR コードのスキャンを続行するには、電話の設定でカメラの許可を手動で変更してください。";
  ...
  static Map<String, Function> _notInlinedMessages(_) => <String, Function> {
    ...
    "Play at full volume" : MessageLookupByLibrary.simpleMessage("フルボリュームで再生"),
    ...
    "a swap runs to completion" : MessageLookupByLibrary.simpleMessage("スワップは完了するまで実行されます"),
    "accepteula" : MessageLookupByLibrary.simpleMessage("EULA に同意する"),
    "accepttac" : MessageLookupByLibrary.simpleMessage("利用規約に同意する"),
    "activateAccessBiometric" : MessageLookupByLibrary.simpleMessage("生体認証保護を有効にする"),
    "activateAccessPin" : MessageLookupByLibrary.simpleMessage("PIN 保護を有効にする"),
    ...
    "allowCustomSeed" : MessageLookupByLibrary.simpleMessage("カスタム シードを許可する"),
    ...
    "answer_2" : m5,
    "answer_3" : m6,
    "answer_4" : MessageLookupByLibrary.simpleMessage("はい。各アトミック スワップを正常に完了するには、インターネットへの接続を維持し、アプリを実行している必要があります (通常、接続が非常に短時間中断しても問題ありません)。そうしないと、Maker の場合は取引キャンセルのリスクがあり、Taker の場合は資金を失うリスクがあります。アトミック スワップ プロトコルでは、両方の参加者がオンライン状態を維持し、関与するブロックチェーンを監視して、プロセスがアトミック状態を維持する必要があります。"),
    "answer_5" : m7,
    "answer_6" : m8,
    "answer_7" : m9,
    "answer_8" : m10,
    ...
    "automaticRedirected" : MessageLookupByLibrary.simpleMessage("アクティベーションの再試行プロセスが完了すると、ポートフォリオ ページに自動的にリダイレクトされます。"),
    ...
    "batterySavingWarning" : MessageLookupByLibrary.simpleMessage("お使いの携帯電話はバッテリー節約モードになっています。このモードを無効にするか、アプリケーションをバックグラウンドにしないでください。そうしないと、アプリが OS によって強制終了され、スワップが失敗する可能性があります。"),
    ...
    "buyOrderType" : MessageLookupByLibrary.simpleMessage("一致しない場合はMakerに変換"),
    "buySuccessWaiting" : MessageLookupByLibrary.simpleMessage("スワップが発行されました。お待ちください!"),
    "buySuccessWaitingError" : m14,
    "buyTestCoinWarning" : MessageLookupByLibrary.simpleMessage("警告、本当の価値のないテスト コインを購入しても構わないと思っています。"),
    "camoPinBioProtectionConflict" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN とバイオ プロテクションを同時に有効にすることはできません。"),
    "camoPinBioProtectionConflictTitle" : MessageLookupByLibrary.simpleMessage("Camo PIN と Bio 保護の競合。"),
    "camoPinChange" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN の変更"),
    "camoPinCreate" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN の作成"),
    "camoPinDesc" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN でアプリのロックを解除すると、偽の LOW 残高が表示され、カモフラージュ PIN 構成オプションは設定に表示されません。"),
    "camoPinInvalid" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN が無効です"),
    "camoPinLink" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN"),
    "camoPinNotFound" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN が見つかりません"),
    "camoPinOff" : MessageLookupByLibrary.simpleMessage("オフ"),
    "camoPinOn" : MessageLookupByLibrary.simpleMessage("の上"),
    "camoPinSaved" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN を保存しました"),
    "camoPinTitle" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN"),
    "camoSetupSubtitle" : MessageLookupByLibrary.simpleMessage("新しいカモフラージュ PIN を入力してください"),
    "camoSetupTitle" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN の設定"),
    "camouflageSetup" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN の設定"),
    ...
    "candleChartError" : MessageLookupByLibrary.simpleMessage("エラーが発生しました。あとでもう一度試してみてください。"),
    "cantDeleteDefaultCoinOk" : MessageLookupByLibrary.simpleMessage("Ok"),
    "cantDeleteDefaultCoinSpan" : MessageLookupByLibrary.simpleMessage("デフォルトのコインです。デフォルトのコインを無効にすることはできません。"),
    "cantDeleteDefaultCoinTitle" : MessageLookupByLibrary.simpleMessage("無効にできません"),
    ...
    "cexDataDesc" : MessageLookupByLibrary.simpleMessage("このアイコンでマークされた市場データ (価格、チャートなど) は、サードパーティのソース (<a href=\"https://www.coingecko.com/\">coingecko.com</a>、<a href=\" https://openrates.io/\">openrates.io</a>)。"),
    ...
    "checkSeedPhrase" : MessageLookupByLibrary.simpleMessage("シード フレーズを確認する"),
    "checkSeedPhraseButton1" : MessageLookupByLibrary.simpleMessage("継続する"),
    "checkSeedPhraseButton2" : MessageLookupByLibrary.simpleMessage("戻ってもう一度確認する"),
    "checkSeedPhraseHint" : m15,
    "checkSeedPhraseInfo" : MessageLookupByLibrary.simpleMessage("シード フレーズは重要です。そのため、正確であることを確認したいと考えています。いつでも簡単にウォレットを復元できるように、シード フレーズについて 3 つの異なる質問をします。"),
    "checkSeedPhraseSubtile" : m16,
    "checkSeedPhraseTitle" : MessageLookupByLibrary.simpleMessage("シードフレーズを再確認しましょう"),
    ...
    "coinSelectNotFound" : MessageLookupByLibrary.simpleMessage("アクティブなコインはありません"),
    ...
    "configureWallet" : MessageLookupByLibrary.simpleMessage("ウォレットを設定しています。お待ちください..."),
    "confirm" : MessageLookupByLibrary.simpleMessage("確認"),
    "confirmCamouflageSetup" : MessageLookupByLibrary.simpleMessage("カモフラージュ PIN の確認"),
    "confirmCancel" : MessageLookupByLibrary.simpleMessage("注文をキャンセルしてもよろしいですか"),
    ...
    "confirmeula" : MessageLookupByLibrary.simpleMessage("下のボタンをクリックすることで、「EULA」と「利用規約」を読んだことを確認し、これらに同意したことになります"),
    ...
    "contactExitWarning" : MessageLookupByLibrary.simpleMessage("変更を破棄しますか?"),
    ...
    "couldntImportError" : MessageLookupByLibrary.simpleMessage("インポートできませんでした:"),
    ...
    "customFeeWarning" : MessageLookupByLibrary.simpleMessage("あなたが何をしているのかを知っている場合にのみ、カスタム料金を使用してください!"),
    ...
    "deleteSpan2" : MessageLookupByLibrary.simpleMessage("ポートフォリオから？一致しない注文はすべてキャンセルされます。"),
    "deleteWallet" : MessageLookupByLibrary.simpleMessage("ウォレットの削除"),
    "deletingWallet" : MessageLookupByLibrary.simpleMessage("ウォレットを削除しています..."),
    ...
    "dexIsNotAvailable" : MessageLookupByLibrary.simpleMessage("このコインではDEXは利用できません"),
    ...
    "emptyCoin" : m19,
    "emptyExportPass" : MessageLookupByLibrary.simpleMessage("暗号化パスワードを空にすることはできません"),
    "emptyImportPass" : MessageLookupByLibrary.simpleMessage("パスワードを空にすることはできません"),
    "emptyName" : MessageLookupByLibrary.simpleMessage("連絡先の名前を空にすることはできません"),
    "emptyWallet" : MessageLookupByLibrary.simpleMessage("ウォレット名を空にすることはできません"),
    "enableTestCoins" : MessageLookupByLibrary.simpleMessage("テストコインを有効にする"),
    "enablingTooManyAssetsSpan1" : MessageLookupByLibrary.simpleMessage("あなたが持っている"),
    "enablingTooManyAssetsSpan2" : MessageLookupByLibrary.simpleMessage("アセットが有効になっており、有効にしようとしています"),
    "enablingTooManyAssetsSpan3" : MessageLookupByLibrary.simpleMessage("もっと。有効なアセットの上限は"),
    "enablingTooManyAssetsSpan4" : MessageLookupByLibrary.simpleMessage(".新しいものを追加する前に、いくつかを無効にしてください。"),
    ...
    "enterSeedPhrase" : MessageLookupByLibrary.simpleMessage("シードフレーズを入力してください"),
    "enterSellAmount" : MessageLookupByLibrary.simpleMessage("最初に販売金額を入力する必要があります"),
    "enterpassword" : MessageLookupByLibrary.simpleMessage("続行するには、パスワードを入力してください。"),
    "errorAmountBalance" : MessageLookupByLibrary.simpleMessage("バランスが足りない"),
    "errorNotAValidAddress" : MessageLookupByLibrary.simpleMessage("有効な住所ではありません"),
    "errorNotAValidAddressSegWit" : MessageLookupByLibrary.simpleMessage("Segwit アドレスはサポートされていません (まだ)"),
    "errorNotEnoughGas" : m20,
    "errorTryAgain" : MessageLookupByLibrary.simpleMessage("エラーです。もう一度お試しください"),
    "errorTryLater" : MessageLookupByLibrary.simpleMessage("エラーです。後で試してください"),
    "errorValueEmpty" : MessageLookupByLibrary.simpleMessage("値が高すぎるか低すぎる"),
    "errorValueNotEmpty" : MessageLookupByLibrary.simpleMessage("データを入力してください"),
    ...
    "exampleHintSeed" : MessageLookupByLibrary.simpleMessage("例: build case level ..."),
    ...
    "exportDesc" : MessageLookupByLibrary.simpleMessage("暗号化ファイルにエクスポートするアイテムを選択してください。"),
    "exportLink" : MessageLookupByLibrary.simpleMessage("書き出す"),
    "exportNotesTitle" : MessageLookupByLibrary.simpleMessage("ノート"),
    "exportSuccessTitle" : MessageLookupByLibrary.simpleMessage("アイテムは正常にエクスポートされました:"),
    ...
    "feedUnableToProceed" : MessageLookupByLibrary.simpleMessage("ニュースの更新を続行できません"),
    "feedUnableToUpdate" : MessageLookupByLibrary.simpleMessage("ニュースの更新を取得できません"),
    ...
    "filtersClearAll" : MessageLookupByLibrary.simpleMessage("すべてのフィルターをクリア"),
    ...
    "generalPinNotActive" : MessageLookupByLibrary.simpleMessage("一般的な PIN 保護はアクティブではありません。カモフラージュモードは利用できません。 PIN 保護を有効にしてください。"),
    "getBackupPhrase" : MessageLookupByLibrary.simpleMessage("重要: 続行する前に、シード フレーズをバックアップしてください。"),
    ...
    "hintConfirmPassword" : MessageLookupByLibrary.simpleMessage("パスワードを認証する"),
    ...
    "hintEnterPassword" : MessageLookupByLibrary.simpleMessage("パスワードを入力してください"),
    "hintEnterSeedPhrase" : MessageLookupByLibrary.simpleMessage("シード フレーズを入力してください"),
    "hintNameYourWallet" : MessageLookupByLibrary.simpleMessage("ウォレットに名前を付ける"),
    ...
    "importDecryptError" : MessageLookupByLibrary.simpleMessage("無効なパスワードまたは破損したデータ"),
    "importDesc" : MessageLookupByLibrary.simpleMessage("輸入する品目:"),
    "importFileNotFound" : MessageLookupByLibrary.simpleMessage("ファイルが見つかりません"),
    "importInvalidSwapData" : MessageLookupByLibrary.simpleMessage("無効なスワップ データです。有効なスワップ ステータス JSON ファイルを提供してください。"),
    "importLink" : MessageLookupByLibrary.simpleMessage("輸入"),
    "importLoadDesc" : MessageLookupByLibrary.simpleMessage("インポートする暗号化ファイルを選択してください。"),
    "importLoadSwapDesc" : MessageLookupByLibrary.simpleMessage("インポートするプレーン テキスト スワップ ファイルを選択してください。"),
    ...
    "importSingleSwapLink" : MessageLookupByLibrary.simpleMessage("シングル スワップのインポート"),
    "importSingleSwapTitle" : MessageLookupByLibrary.simpleMessage("輸入スワップ"),
    "importSomeItemsSkippedWarning" : MessageLookupByLibrary.simpleMessage("一部のアイテムがスキップされました"),
    "importSuccessTitle" : MessageLookupByLibrary.simpleMessage("アイテムが正常にインポートされました:"),
    "importSwapFailed" : MessageLookupByLibrary.simpleMessage("スワップのインポートに失敗しました"),
    "importSwapJsonDecodingError" : MessageLookupByLibrary.simpleMessage("json ファイルのデコード中にエラーが発生しました"),
    "importTitle" : MessageLookupByLibrary.simpleMessage("輸入"),
    "infoPasswordDialog" : MessageLookupByLibrary.simpleMessage("安全なパスワードを使用し、同じデバイスに保存しないでください"),
    "infoTrade1" : MessageLookupByLibrary.simpleMessage("スワップリクエストは元に戻すことができず、最終イベントです!"),
    "infoTrade2" : MessageLookupByLibrary.simpleMessage("スワップには最大 60 分かかる場合があります。このアプリケーションを閉じないでください！"),
    "infoWalletPassword" : MessageLookupByLibrary.simpleMessage("セキュリティ上の理由から、ウォレット暗号化用のパスワードを提供する必要があります。"),
    "insufficientBalanceToPay" : m39,
    "insufficientText" : MessageLookupByLibrary.simpleMessage("この注文に必要な最小ボリュームは"),
    ...
    "internetRestored" : MessageLookupByLibrary.simpleMessage("インターネット接続が回復しました"),
    ...
    "loadingOrderbook" : MessageLookupByLibrary.simpleMessage("オーダーブックを読み込んでいます..."),
    ...
    "logoutWarning" : MessageLookupByLibrary.simpleMessage("今すぐログアウトしてもよろしいですか?"),
    ...
    "makerDetailsCancel" : MessageLookupByLibrary.simpleMessage("注文をキャンセルする"),
    ...
    "makerDetailsNoSwaps" : MessageLookupByLibrary.simpleMessage("この注文で開始されたスワップはありません"),
    "makerDetailsPrice" : MessageLookupByLibrary.simpleMessage("価格"),
    "makerDetailsSell" : MessageLookupByLibrary.simpleMessage("売る"),
    "makerDetailsSwaps" : MessageLookupByLibrary.simpleMessage("この注文で開始されたスワップ"),
    ...
    "marketsSelectCoins" : MessageLookupByLibrary.simpleMessage("コインを選択してください"),
    "marketsTab" : MessageLookupByLibrary.simpleMessage("マーケット"),
    "marketsTitle" : MessageLookupByLibrary.simpleMessage("市場"),
    "matchExportPass" : MessageLookupByLibrary.simpleMessage("パスワードが一致する必要があります"),
    "matchingCamoChange" : MessageLookupByLibrary.simpleMessage("変化する"),
    "matchingCamoPinError" : MessageLookupByLibrary.simpleMessage("一般暗証番号とカモフラージュ暗証番号は同じです。カモフラージュモードは利用できません。カモフラージュ PIN を変更してください。"),
    ...
    "mediaNotSavedDescription" : MessageLookupByLibrary.simpleMessage("保存した記事はありません"),
    ...
    "minValueSell" : m44,
    "minVolumeInput" : m45,
    "minVolumeIsTDH" : MessageLookupByLibrary.simpleMessage("販売額より低くなければなりません"),
    "minVolumeTitle" : MessageLookupByLibrary.simpleMessage("必要な最小ボリューム"),
    "minVolumeToggle" : MessageLookupByLibrary.simpleMessage("カスタム最小ボリュームを使用"),
    ...
    "multiFiatDesc" : MessageLookupByLibrary.simpleMessage("受け取る金額を入力してください:"),
    "multiFiatFill" : MessageLookupByLibrary.simpleMessage("オートフィル"),
    "multiFixErrors" : MessageLookupByLibrary.simpleMessage("続行する前にすべてのエラーを修正してください"),
    "multiInvalidAmt" : MessageLookupByLibrary.simpleMessage("金額が無効です"),
    "multiInvalidSellAmt" : MessageLookupByLibrary.simpleMessage("販売金額が無効です"),
    "multiLowGas" : m49,
    ...
    "noArticles" : MessageLookupByLibrary.simpleMessage("ニュースはありません。後でもう一度確認してください。"),
    "noCoinFound" : MessageLookupByLibrary.simpleMessage("コインが見つかりません"),
    "noFunds" : MessageLookupByLibrary.simpleMessage("資金がない"),
    "noFundsDetected" : MessageLookupByLibrary.simpleMessage("資金がありません - 入金してください。"),
    ...
    "noMatchingOrders" : MessageLookupByLibrary.simpleMessage("一致する注文が見つかりません"),
    "noOrder" : m51,
    "noOrderAvailable" : MessageLookupByLibrary.simpleMessage("クリックして注文を作成"),
    "noOrders" : MessageLookupByLibrary.simpleMessage("注文はありません。取引に行ってください。"),
    "noRewardYet" : MessageLookupByLibrary.simpleMessage("報酬は請求できません - 1 時間後にもう一度お試しください。"),
    ...
    "nonNumericInput" : MessageLookupByLibrary.simpleMessage("値は数値でなければなりません"),
    "notEnoughGas" : m52,
    "notEnoughtBalanceForFee" : MessageLookupByLibrary.simpleMessage("手数料に十分な残高がありません - 少額で取引してください"),
    "noteOnOrder" : MessageLookupByLibrary.simpleMessage("注: 一致した注文は再度キャンセルできません"),
    ...
    "notifSwapCompletedTitle" : MessageLookupByLibrary.simpleMessage("スワップ完了"),
    "notifSwapFailedText" : m54,
    "notifSwapFailedTitle" : MessageLookupByLibrary.simpleMessage("スワップに失敗しました"),
    "notifSwapStartedText" : m55,
    "notifSwapStartedTitle" : MessageLookupByLibrary.simpleMessage("新しいスワップが開始されました"),
    "notifSwapStatusTitle" : MessageLookupByLibrary.simpleMessage("スワップ ステータスが変更されました"),
    "notifSwapTimeoutText" : m56,
    "notifSwapTimeoutTitle" : MessageLookupByLibrary.simpleMessage("スワップ タイムアウト"),
    "notifTxText" : m57,
    ...
    "orderCreatedInfo" : MessageLookupByLibrary.simpleMessage("注文が正常に作成されました"),
    ...
    "orderDetailsSettings" : MessageLookupByLibrary.simpleMessage("シングルタップで詳細を開き、ロングタップで注文を選択します"),
    ...
    "paidFromVolume" : MessageLookupByLibrary.simpleMessage("受け取ったボリュームから支払われる:"),
    "paidWith" : MessageLookupByLibrary.simpleMessage("で支払った"),
    "passwordRequirement" : MessageLookupByLibrary.simpleMessage("パスワードには、小文字 1 つ、大文字 1 つ、特殊記号 1 つを含む 12 文字以上を含める必要があります。"),
    "paymentUriDetailsAccept" : MessageLookupByLibrary.simpleMessage("支払い"),
    "paymentUriDetailsAcceptQuestion" : MessageLookupByLibrary.simpleMessage("この取引を受け入れますか？"),
    "paymentUriDetailsAddressSpan" : MessageLookupByLibrary.simpleMessage("住所へ"),
    "paymentUriDetailsAmountSpan" : MessageLookupByLibrary.simpleMessage("額："),
    "paymentUriDetailsCoinSpan" : MessageLookupByLibrary.simpleMessage("コイン："),
    "paymentUriDetailsDeny" : MessageLookupByLibrary.simpleMessage("キャンセル"),
    "paymentUriDetailsTitle" : MessageLookupByLibrary.simpleMessage("支払いが要求されました"),
    ...
    "pleaseRestart" : MessageLookupByLibrary.simpleMessage("アプリを再起動してもう一度試すか、下のボタンを押してください。"),
    ...
    "protectionCtrlConfirmations" : MessageLookupByLibrary.simpleMessage("確認"),
    "protectionCtrlCustom" : MessageLookupByLibrary.simpleMessage("カスタム保護設定を使用する"),
    "protectionCtrlOff" : MessageLookupByLibrary.simpleMessage("オフ"),
    "protectionCtrlOn" : MessageLookupByLibrary.simpleMessage("オン"),
    "protectionCtrlWarning" : MessageLookupByLibrary.simpleMessage("警告、このアトミック スワップは dPoW で保護されていません。"),
    "pubkey" : MessageLookupByLibrary.simpleMessage("公開鍵"),
    "question_1" : MessageLookupByLibrary.simpleMessage("私の秘密鍵を保管しますか?"),
    "question_10" : m67,
    "question_2" : m68,
    "question_3" : MessageLookupByLibrary.simpleMessage("各アトミック スワップにはどのくらいの時間がかかりますか?"),
    "question_4" : MessageLookupByLibrary.simpleMessage("スワップ中はオンラインである必要がありますか?"),
    "question_5" : m69,
    "question_6" : MessageLookupByLibrary.simpleMessage("ユーザーサポートは提供していますか？"),
    ...
    "recommendSeedMessage" : MessageLookupByLibrary.simpleMessage("オフラインで保存することをお勧めします。"),
    ...
    "retryActivating" : MessageLookupByLibrary.simpleMessage("すべてのコインの有効化を再試行しています..."),
    ...
    "rewardsError" : MessageLookupByLibrary.simpleMessage("エラーが発生しました。後でもう一度やり直してください。"),
    "rewardsInProgressLong" : MessageLookupByLibrary.simpleMessage("取引が進行中です"),
    "rewardsInProgressShort" : MessageLookupByLibrary.simpleMessage("処理"),
    "rewardsLowAmountLong" : MessageLookupByLibrary.simpleMessage("UTXOの金額が10KMD未満"),
    "rewardsLowAmountShort" : MessageLookupByLibrary.simpleMessage("<10 KMD"),
    "rewardsOneHourLong" : MessageLookupByLibrary.simpleMessage("まだ1時間が経過していない"),
    ...
    "rewardsReadMore" : MessageLookupByLibrary.simpleMessage("KMDアクティブユーザー特典について詳しく見る"),
    ...
    "searchFilterSubtitleAVX" : MessageLookupByLibrary.simpleMessage("すべての Avax トークンを選択"),
    "searchFilterSubtitleBEP" : MessageLookupByLibrary.simpleMessage("すべての BEP トークンを選択"),
    "searchFilterSubtitleERC" : MessageLookupByLibrary.simpleMessage("すべてのERCトークンを選択"),
    "searchFilterSubtitleETC" : MessageLookupByLibrary.simpleMessage("すべてのETCトークンを選択"),
    "searchFilterSubtitleFTM" : MessageLookupByLibrary.simpleMessage("すべてのファントムトークンを選択"),
    "searchFilterSubtitleHCO" : MessageLookupByLibrary.simpleMessage("すべての HecoChain トークンを選択"),
    "searchFilterSubtitleHRC" : MessageLookupByLibrary.simpleMessage("すべてのハーモニートークンを選択"),
    "searchFilterSubtitleKRC" : MessageLookupByLibrary.simpleMessage("すべての Kucoin トークンを選択"),
    "searchFilterSubtitleMVR" : MessageLookupByLibrary.simpleMessage("ムーンリバートークンをすべて選択"),
    "searchFilterSubtitlePLG" : MessageLookupByLibrary.simpleMessage("すべてのポリゴン トークンを選択する"),
    "searchFilterSubtitleQRC" : MessageLookupByLibrary.simpleMessage("すべての QRC トークンを選択"),
    "searchFilterSubtitleSBCH" : MessageLookupByLibrary.simpleMessage("すべての SmartBCH トークンを選択"),
    "searchFilterSubtitleSmartChain" : MessageLookupByLibrary.simpleMessage("すべての SmartChain を選択"),
    "searchFilterSubtitleTestCoins" : MessageLookupByLibrary.simpleMessage("すべてのテスト アセットを選択"),
    "searchFilterSubtitleUBQ" : MessageLookupByLibrary.simpleMessage("すべての Ubiq コインを選択"),
    "searchFilterSubtitleutxo" : MessageLookupByLibrary.simpleMessage("すべてのUTXOコインを選択"),
    ...
    "seedPhraseTitle" : MessageLookupByLibrary.simpleMessage("あなたの新しいシードフレーズ"),
    "selectCoin" : MessageLookupByLibrary.simpleMessage("コインを選択"),
    "selectCoinInfo" : MessageLookupByLibrary.simpleMessage("ポートフォリオに追加するコインを選択します。"),
    ...
    "selectPaymentMethod" : MessageLookupByLibrary.simpleMessage("お支払い方法を選択してください"),
    "selectedOrder" : MessageLookupByLibrary.simpleMessage("選択した注文:"),
    "sell" : MessageLookupByLibrary.simpleMessage("売る"),
    "sellTestCoinWarning" : MessageLookupByLibrary.simpleMessage("警告、実際の価値のないテスト コインを販売しても構わないと思っています。"),
    "send" : MessageLookupByLibrary.simpleMessage("送信"),
    "setUpPassword" : MessageLookupByLibrary.simpleMessage("パスワードを設定する"),
    "settingDialogSpan1" : MessageLookupByLibrary.simpleMessage("消去してもよろしいですか"),
    "settingDialogSpan2" : MessageLookupByLibrary.simpleMessage("財布？"),
    "settingDialogSpan3" : MessageLookupByLibrary.simpleMessage("もしそうなら、あなたを確認してください"),
    "settingDialogSpan4" : MessageLookupByLibrary.simpleMessage("シード フレーズを記録します。"),
    "settingDialogSpan5" : MessageLookupByLibrary.simpleMessage("将来的にウォレットを復元するため。"),
    ...
    "signInWithPassword" : MessageLookupByLibrary.simpleMessage("パスワードでサインイン"),
    "signInWithSeedPhrase" : MessageLookupByLibrary.simpleMessage("パスワードをお忘れですか？シードからウォレットを復元する"),
    ...
    "soundsDoNotShowAgain" : MessageLookupByLibrary.simpleMessage("了解しました もう表示しないでください"),
    "soundsExplanation" : MessageLookupByLibrary.simpleMessage("スワップ プロセス中、およびアクティブなメーカー注文が行われると、サウンド通知が聞こえます。アトミック スワップ プロトコルでは、取引を成功させるには参加者がオンラインである必要があり、サウンド通知はそれを達成するのに役立ちます。"),
    "soundsNote" : MessageLookupByLibrary.simpleMessage("アプリケーション設定でカスタムサウンドを設定できることに注意してください。"),
    ...
    "tooManyAssetsEnabledSpan1" : MessageLookupByLibrary.simpleMessage("あなたが持っている"),
    "tooManyAssetsEnabledSpan2" : MessageLookupByLibrary.simpleMessage("アセットが有効になっています。有効なアセットの上限は"),
    "tooManyAssetsEnabledSpan3" : MessageLookupByLibrary.simpleMessage(".新しいものを追加する前に、いくつかを無効にしてください。"),
    "tooManyAssetsEnabledTitle" : MessageLookupByLibrary.simpleMessage("有効なアセットが多すぎます"),
    ...
    "tradePreimageError" : MessageLookupByLibrary.simpleMessage("取引手数料の計算に失敗しました"),
    "tradingFee" : MessageLookupByLibrary.simpleMessage("取引手数料:"),
    "tradingMode" : MessageLookupByLibrary.simpleMessage("取引モード:"),
    "traditionalChinese" : MessageLookupByLibrary.simpleMessage("伝統的"),
    "tryRestarting" : MessageLookupByLibrary.simpleMessage("それでも一部のコインが有効化されない場合は、アプリを再起動してみてください。"),
    ...
    "txLimitExceeded" : MessageLookupByLibrary.simpleMessage("リクエストが多すぎます。トランザクション履歴リクエストの制限を超えました。後でもう一度やり直してください。"),
    ...
    "updatesAvailable" : MessageLookupByLibrary.simpleMessage("新しいバージョンが利用可能"),
    "updatesChecking" : MessageLookupByLibrary.simpleMessage("アップデートの確認..."),
    "updatesCurrentVersion" : m91,
    "updatesNotifAvailable" : MessageLookupByLibrary.simpleMessage("新しいバージョンが利用可能です。更新してください。"),
    "updatesNotifAvailableVersion" : m92,
    "updatesNotifTitle" : MessageLookupByLibrary.simpleMessage("利用可能なアップデート"),
    ...
    "uriInsufficientBalanceSpan1" : MessageLookupByLibrary.simpleMessage("スキャンするのに十分な残高がありません"),
    "uriInsufficientBalanceSpan2" : MessageLookupByLibrary.simpleMessage("支払い要求。"),
    "uriInsufficientBalanceTitle" : MessageLookupByLibrary.simpleMessage("残高不足"),
    ...
    "walletInUse" : MessageLookupByLibrary.simpleMessage("ウォレット名はすでに使用されています"),
    "walletMaxChar" : MessageLookupByLibrary.simpleMessage("ウォレット名は最大 40 文字にする必要があります"),
    "walletOnly" : MessageLookupByLibrary.simpleMessage("ウォレットのみ"),
    "warning" : MessageLookupByLibrary.simpleMessage("警告！"),
    "warningOkBtn" : MessageLookupByLibrary.simpleMessage("Ok"),
    "warningShareLogs" : MessageLookupByLibrary.simpleMessage("警告 - 特殊なケースでは、このログ データには機密情報が含まれており、失敗したスワップからのコインの使用に使用できる可能性があります。"),
    "weFailedTo" : m94,
    ...
    "willBeRedirected" : MessageLookupByLibrary.simpleMessage("完了すると、ポートフォリオ ページにリダイレクトされます。"),
    "withdraw" : MessageLookupByLibrary.simpleMessage("撤退"),
    "withdrawCameraAccessText" : m97,
    "withdrawCameraAccessTitle" : MessageLookupByLibrary.simpleMessage("アクセスが拒否されました"),
    "withdrawConfirm" : MessageLookupByLibrary.simpleMessage("出金確認"),
    "withdrawConfirmError" : MessageLookupByLibrary.simpleMessage("エラーが発生しました。あとでもう一度試してみてください。"),
    "withdrawValue" : m98,
    "wrongCoinSpan1" : MessageLookupByLibrary.simpleMessage("の支払い QR コードをスキャンしようとしています"),
    ...
    "wrongPassword" : MessageLookupByLibrary.simpleMessage("パスワードが一致しません。もう一度やり直してください。"),
    "yes" : MessageLookupByLibrary.simpleMessage("はい"),
    "you have a fresh order that is trying to match with an existing order" : MessageLookupByLibrary.simpleMessage("既存の注文と一致させようとしている新しい注文があります"),
    "you have an active swap in progress" : MessageLookupByLibrary.simpleMessage("アクティブなスワップが進行中です"),
    "you have an order that new orders can match with" : MessageLookupByLibrary.simpleMessage("新しい注文と一致する注文があります"),
