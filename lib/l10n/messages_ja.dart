...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m73(seconde) => "オーダーマッチ中、あと${seconde} 秒お待ち下さい。\n\n";

  static m1(index) => "${index}番目の単語を入力";

  static m2(index) => "パスフレーズの${index}番目の単語は何ですか？";
  ...
  static m41(coinName, number) => "売却最小額は ${number} ${coinName} です。";

  static m42(coinName, number) => "購入最小額は ${number} ${coinName}です。\n";

  static m17(coinName) => "利用可能な${coinName}の注文がありません - 暫く経ってから再度試すか注文を作成して下さい。";

  static m18(assets) => "${assets} アセット";

  static m19(amount) => "クリックして ${amount} オーダーを確認します";

  static m20(coinName, address) => "私の ${coinName} アドレス: ${address}";

  static m21(appName) => "AtomicDEXモバイルは第三世代のDEX機能を搭載した次世代型のマルチコインウォレットです。";

  static m22(amount, coinName) => "出金 ${amount} ${coinName}";

  static m23(amount, coin) => "あなたは${amount} ${coin}受け取ります";
