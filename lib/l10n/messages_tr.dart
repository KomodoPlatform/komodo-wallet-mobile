...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m0(name) => "${name} başarıyla etkinleştirildi !";

  static m1(title) => "Yalnızca ${title} adresli kişiler gösteriliyor";

  static m2(abbr) => "${abbr} etkinleştirilmediği için ${abbr} adresine para gönderemezsiniz. Lütfen portföye gidiniz.";

  static m3(appName) => "Hayır ! AtomicDEX, gözetimsiz bir cüzdandır. Özel kelimeleriniz, gizli kelimeleriniz ve PIN kodunuz dahil hiçbir hassas bilgiyi kaydetmiyoruz. Bu bilgiler sadece kullanıcının cihazında tutulmaktadır ve başka bir yere gitmez. Bu sayede koin ve tokenlerinizin tüm kontrolü sizdedir.";
  ...
  static m8(appName) => "Hayır ! ${appName} tamamıyla merkeziyetsizdir ve kullanıcıların uygulamaya erişimi başkaları tarafından sınırlandırılamaz.";

  static m9(appName, appCompanyShort) => "${appName}, ${appCompanyShort} takımı tarafından geliştirilmiştir. ${appCompanyShort}, koin takası, Geciktirilmiş İş Kanıtı (dPoW), birlikte çalışabilen çoklu zincir mimarisi gibi yenilikçi blokzincir çözümleri geliştiren köklü bir platformdur.";

  static m10(appName) => "Tabii ki ! Daha fazla bilgi için <a href=\"https://developers.atomicdex.io/\">developer documentation</a> dökümanımızı okuyabilir ya da iş birliği teklifleriniz için bizimle iletişime geçebilirsiniz. Özel bir teknik sorunuz mu var ? ${appName}  geliştirici topluluğu yardımcı olmak için her daim hazırdır.";

  static m11(batteryLevelCritical) => "Telefonunuzun bataryası güvenli bir takası tamamlayamayacak (${batteryLevelCritical}%) kadar düşük. Lütfen önce şarja takıp sonra tekrar deneyiniz.";

  static m12(batteryLevelLow) => "Telefonunuzun bataryası %${batteryLevelLow}\'den daha az. Lütfen telefonunuzu şarj ediniz.";

  static m13(seconde) => "Emir eşleşmesi devam ediyor, lütfen ${seconde} saniye kadar bekleyiniz.";

  static m14(index) => "${index} kelimesini giriniz.";

  static m15(index) => "Gizli kelimeleriniz arasından ${index} olanı hangisidir ?";

  static m16(name) => "${name} kişisini silmek istediğinizden emin misiniz ?";

  static m17(iUnderstand) => "Özelleştirilmiş gizli kelime kullanımı yeterince güvenli olmayabilir ve uygulama tarafından oluşturulan BIP39 uyumlu gizli kelimeler ya da özel anahtara (WIF) kıyasla daha kolay kırılabilmektedir. Riskleri kabul edip ne yaptığınızdan emin olduğunuzu tasdiklemek için aşağıdaki kutucuğa \"${iUnderstand}\" yazınız.";

  static m18(abbr) => "${abbr} adresini girin";

  static m19(gas) => "Yeteri kadar gaz ücreti yok, en az ${gas} Gwei gerekli";
  ...
  static m34(coin) => "${coin} musluğuna talep gönderiliyor..";

  static m35(appCompanyShort) => "${appCompanyShort} haberleri";

  static m36(coin) => "${coin} gideri";

  static m37(coin) => "Lütfen ${coin} koinini aktifleştirin.";

  static m38(abbr) => "${abbr} bakiyesi, alım satım işlem ücretlerini karşılamaya yetmiyor.";

  static m39(coinAbbr) => "${coinAbbr} mevcut değil :(";

  static m40(coinName, number) => "${coinName} için yapılabilecek en düşük satış hacmi: ${number}";

  static m41(coinName, number) => "${coinName} için yapılabilecek en düşük alım hacmi: ${number}";

  static m42(buyCoin, buyAmount, sellCoin, sellAmount) => "En düşük emir için: ${buyCoin} ${buyAmount}\n(${sellCoin} ${sellAmount})";

  static m43(coinName, number) => "${coinName} için en düşük satım miktarı: ${number}";

  static m44(minValue, coin) => "${minValue} ${coin}\'den büyük olmalı";

  static m45(appName) => "Lütfen artık hücresel veri kullandığınızı ve ${appName} P2P ağına katılımınızın internet trafiğini tükettiğini unutmayın. Hücresel veri planınız maliyetliyse bir WiFi ağı kullanmak daha iyidir.";

  static m46(coin) => "Önce ${coin}\'i etkinleştirin ve bakiyeyi tamamlayın";

  static m47(number) => "${number} Sipariş oluştur:";

  static m48(coin) => "${coin} bakiyesi yetersiz";

  static m49(coin, fee) => "İşlem masrafını ödemeye yetecek ${coin} yok. En az ${fee} ${coin} olmalı.";

  static m50(coinName) => "Uygun bir ${coinName} emri bulunmuyor - bir emir girmeyi deneyin veya daha sonra yeniden kontrol edin.";

  static m51(coin) => "İşlem için yeteri kadar ${coin} yok !";

  static m52(sell, buy) => "${sell}/${buy} takası başarıyla tamamlandı";

  static m53(sell, buy) => "${sell}/${buy} takası başarısız oldu";

  static m54(sell, buy) => "${sell}/${buy} takası başladı";

  static m55(sell, buy) => "${sell}/${buy} takas zaman aşımına uğradı";

  static m56(coin) => "${coin} işlemi aldınız!";

  static m57(assets) => "${assets} Varlıklar";

  static m58(coin) => "Tüm ${coin} emirleri iptal edilecek";

  static m59(delta) => "Ucuz: CEX + %${delta}";

  static m60(delta) => "Pahalı: CEX + %${delta}";

  static m61(fill) => "${fill}% tamamlandı";

  static m62(coin) => "(${coin}) miktarı";

  static m63(coin) => "(${coin}) fiyatı";

  static m64(coin) => "Toplam (${coin})";

  static m65(abbr) => "${abbr} aktif değil. Lütfen aktifleştirip öyle deneyiniz.";

  static m66(appName) => "AtomicDEX\'i hangi cihazlarda kullanabilirim ?";

  static m67(appName) => "${appName}\'te alım satım yapmanın diğer DEX\'lerdekinden ne gibi farkları vardır ?";

  static m68(appName) => "${appName}\'te işlem ücretleri nasıl hesaplanmaktadır ?";

  static m69(appName) => "AtomicDEX\'in arkasında kimler var ?";

  static m70(appName) => "${appName} üzerinde kendi beyaz etiketli değişimimi geliştirmem mümkün mü?";

  static m71(amount) => "Başarılı ! ${amount} KMD geldi.";

  static m72(dd) => "${dd} gün";

  static m73(hh, minutes) => "${hh}sa ${minutes}dk";

  static m74(mm) => "${mm}dk";

  static m75(amount) => "${amount} adet emri görmek için tıklayın";

  static m76(coinName, address) => "${coinName} adresim:\n${address}";

  static m77(coin) => "Lütfen alınacak ${coin} adetini girin";

  static m78(maxCoins) => "Maksimum aktif jeton sayısı ${maxCoins}\'dir. Lütfen bazılarını devre dışı bırakın.";

  static m79(coin) => "${coin} koini aktif değil !";

  static m80(coin) => "Lütfen satmak için ${coin} miktarı girin";

  static m81(coin) => "${coin} aktifleştirilemedi";

  static m82(description) => "Lütfen bir mp3 ya da waw dosyası seçin. ${description}\'de ses dosyalarını oynatacağız.";

  static m83(description) => "${description}\'de oynatıldı.";

  static m84(appName) => "Herhangi bir sorunuz varsa veyahut ${appName} uygulamasıyla ile ilgili teknik bir hata bulduğunuzu düşünüyorsanız bunu ekibimize bildirebilir ve destek alabilirsiniz.";

  static m85(coin) => "Lütfen önce ${coin} ve kontör bakiyesini etkinleştirin";

  static m86(coin) => "${coin} bakiyesi, işlem ücretlerini karşılayamayacak kadar düşük.";

  static m87(coin, amount) => "${coin} bakiyesi, işlem ücretlerini karşılayamayacak kadar düşük. ${amount} ${coin} gerekli.";

  static m88(left) => "Kalan işlem: ${left}";

  static m89(amnt, hash) => "${amnt} fonun kilidi başarıyla kaldırıldı - TX: ${hash}";

  static m90(version) => "${version} sürümünü kullanmaktasınız.";

  static m91(version) => "Güncel ${version} sürümü mevcut. Lütfen güncelleyiniz.";

  static m92(appName) => "AtomicDEX güncellemeleri";

  static m93(coinAbbr) => "${coinAbbr} koinini etkinleştiremedik.";

  static m94(coinAbbr) => "${coinAbbr} koinini etkinleştiremedik.\nLütfen uygulamayı yeniden başlatıp tekrar deneyiniz.";

  static m95(appName) => "AtomicDEX mobil yerleşik üçüncü nesil DEX işlevselliği ve daha fazla özellikleri ile yeni nesil bir çoklu koin cüzdanıdır.";

  static m96(appName) => "${appName}\'in kameraya erişimi engellenmiş.\nQR kod taramasını yapabilmek için lütfen telefon ayarlarınızdan kamera erişimine izin veriniz.";

  static m97(amount, coinName) => "${amount} adet ${coinName} ÇEK";

  static m98(amount, coin) => "${amount} adet ${coin} alacaksınız";
  ...
  static Map<String, Function> _notInlinedMessages(_) => <String, Function> {
    ...
    "Can\'t play that" : MessageLookupByLibrary.simpleMessage("Bu oynatılamıyor"),
    "Failed" : MessageLookupByLibrary.simpleMessage("Başarısız"),
    "Maker" : MessageLookupByLibrary.simpleMessage("Yapıcı Emri"),
    "Play at full volume" : MessageLookupByLibrary.simpleMessage("Tam seste oynat"),
    "Sound" : MessageLookupByLibrary.simpleMessage("Ses"),
    "Taker" : MessageLookupByLibrary.simpleMessage("Alıcı Emri"),
    "a swap fails" : MessageLookupByLibrary.simpleMessage("Takas başarısız oldu"),
    "a swap runs to completion" : MessageLookupByLibrary.simpleMessage("takas tamamlanmak üzere"),
    "accepteula" : MessageLookupByLibrary.simpleMessage("EULA\'yı Kabul Et"),
    "accepttac" : MessageLookupByLibrary.simpleMessage("ŞARTLARI ve KOŞULLARI Kabul Et"),
    "activateAccessBiometric" : MessageLookupByLibrary.simpleMessage("Biyometrik korumayı etkinleştir"),
    "activateAccessPin" : MessageLookupByLibrary.simpleMessage("PIN korumasını etkinleştir"),
    ...
    "addressBookEmpty" : MessageLookupByLibrary.simpleMessage("Adres defteri boş"),
    "addressBookFilter" : m1,
    "addressBookTitle" : MessageLookupByLibrary.simpleMessage("Adres Defteri"),
    "addressCoinInactive" : m2,
    "addressNotFound" : MessageLookupByLibrary.simpleMessage("Bir şey bulunamadı"),
    ...
    "allowCustomSeed" : MessageLookupByLibrary.simpleMessage("Kişisel kelimeleri kullan"),
    ...
    "automaticRedirected" : MessageLookupByLibrary.simpleMessage("Tekrar aktivasyon işlemi bittiğinde doğrudan portföy sayfasına yönlendirileceksiniz."),
    "availableVolume" : MessageLookupByLibrary.simpleMessage("maksimum hacim"),
    ...
    "buyOrderType" : MessageLookupByLibrary.simpleMessage("Eşleşme olmazsa Yapıcı Emrine çevir"),
    "buySuccessWaiting" : MessageLookupByLibrary.simpleMessage("Takas işlendi, lütfen bekleyiniz."),
    "buySuccessWaitingError" : m13,
    "buyTestCoinWarning" : MessageLookupByLibrary.simpleMessage("Dikkat ! Test koinlerini herhangi bir değer olmadan almak üzeresiniz."),
    "camoPinBioProtectionConflict" : MessageLookupByLibrary.simpleMessage("Kamuflajlı PIN ve Biyometrik koruma aynı anda etkinleştirilemez."),
    ...
    "camoPinSaved" : MessageLookupByLibrary.simpleMessage("Kamuflajlı PIN kaydedildi"),
    "camoPinTitle" : MessageLookupByLibrary.simpleMessage("Kamuflajlı PIN"),
    "camoSetupSubtitle" : MessageLookupByLibrary.simpleMessage("Yeni bir kamuflajlı PIN girin"),
    "camoSetupTitle" : MessageLookupByLibrary.simpleMessage("Kamuflajlı PIN Kurulumu"),
    "camouflageSetup" : MessageLookupByLibrary.simpleMessage("Kamuflajlı PIN Kurulumu"),
    ...
    "candleChartError" : MessageLookupByLibrary.simpleMessage("Bir hata oluştu, lütfen daha sonra tekrar deneyiniz."),
    "cantDeleteDefaultCoinOk" : MessageLookupByLibrary.simpleMessage("Tamam"),
    "cantDeleteDefaultCoinSpan" : MessageLookupByLibrary.simpleMessage("Ön tanımlı bir koindir. Ön tanımlı koinler kaldırılamaz"),
    "cantDeleteDefaultCoinTitle" : MessageLookupByLibrary.simpleMessage("Kaldırılamaz"),
    ...
    "cexDataDesc" : MessageLookupByLibrary.simpleMessage("Bu simgeyle işaretlenen piyasa verileri (fiyatlar, çizelgeler vb.) üçüncü taraf kaynaklardan (<a href=\"https://www.coingecko.com/\">coingecko.com</a>, <a href=\" https://openrates.io/\">openrates.io</a>)."),
    "changePin" : MessageLookupByLibrary.simpleMessage("PIN kodunu değiştir"),
    "checkForUpdates" : MessageLookupByLibrary.simpleMessage("Güncellemeleri kontrol et"),
    "checkOut" : MessageLookupByLibrary.simpleMessage("Ödeme"),
    "checkSeedPhrase" : MessageLookupByLibrary.simpleMessage("Gizli kelimeleri kontrol edin"),
    "checkSeedPhraseButton1" : MessageLookupByLibrary.simpleMessage("DEVAM"),
    "checkSeedPhraseButton2" : MessageLookupByLibrary.simpleMessage("GERİ DÖN VE KONTROL ET"),
    "checkSeedPhraseHint" : m14,
    "checkSeedPhraseInfo" : MessageLookupByLibrary.simpleMessage("Gizli kelimeleriniz çok önemlidir, bu yüzden onları doğru kaydettiğinizden emin olmak istiyoruz. Cüzdanınızı istediğiniz zaman kolayca geri yükleyebilmeniz için gizli kelimeleriniz hakkında size şimdi üç farklı soru soracağız."),
    "checkSeedPhraseSubtile" : m15,
    "checkSeedPhraseTitle" : MessageLookupByLibrary.simpleMessage("GİZLİ KELİMELERİZİ BİR KEZ DAHA KONTROL EDİNİZ"),
    "chineseLanguage" : MessageLookupByLibrary.simpleMessage("Çince"),
    "claim" : MessageLookupByLibrary.simpleMessage("talep et"),
    "claimTitle" : MessageLookupByLibrary.simpleMessage("KMD ödülünüzü alın"),
    ...
    "closeMessage" : MessageLookupByLibrary.simpleMessage("Hata mesajını kapat"),
    "closePreview" : MessageLookupByLibrary.simpleMessage("Ön izlemeyi kapat"),
    "code" : MessageLookupByLibrary.simpleMessage("Kod:"),
    "coinSelectClear" : MessageLookupByLibrary.simpleMessage("Temizle"),
    "coinSelectNotFound" : MessageLookupByLibrary.simpleMessage("Aktif koin yok"),
    "coinSelectTitle" : MessageLookupByLibrary.simpleMessage("Koin Seç"),
    "comingSoon" : MessageLookupByLibrary.simpleMessage("Yakında..."),
    "commingsoon" : MessageLookupByLibrary.simpleMessage("TX detayları yakında !"),
    "commingsoonGeneral" : MessageLookupByLibrary.simpleMessage("Detaylar yakında !"),
    "commissionFee" : MessageLookupByLibrary.simpleMessage("komisyon ücreti"),
    "comparedToCex" : MessageLookupByLibrary.simpleMessage("CEX ile kıyaslanınca"),
    "configureWallet" : MessageLookupByLibrary.simpleMessage("Cüzdanınız ayarlanıyor, lütfen bekleyiniz.."),
    "confirm" : MessageLookupByLibrary.simpleMessage("Onayla"),
    "confirmCamouflageSetup" : MessageLookupByLibrary.simpleMessage("Kamuflajlı PIN\'i Onaylayın"),
    "confirmCancel" : MessageLookupByLibrary.simpleMessage("Emri iptal etmek istediğinizden emin misiniz ?"),
    "confirmPassword" : MessageLookupByLibrary.simpleMessage("Parolayı onayla"),
    "confirmPin" : MessageLookupByLibrary.simpleMessage("PIN kodunu onayla"),
    "confirmSeed" : MessageLookupByLibrary.simpleMessage("Gizli Kelimeleri Onayla"),
    "confirmeula" : MessageLookupByLibrary.simpleMessage("Aşağıdaki düğmeye tıklayarak \'EULA\' ve \'Kullanım Şartları\'nı okumuş ve kabul etmiş sayılırsınız"),
    ...
    "contactExitWarning" : MessageLookupByLibrary.simpleMessage("Değişiklikleri kaydetme ?"),
    "contactNotFound" : MessageLookupByLibrary.simpleMessage("Kişi bulunamadı"),
    ...
    "couldntImportError" : MessageLookupByLibrary.simpleMessage("İçeri alınamıyor"),
    ...
    "customFee" : MessageLookupByLibrary.simpleMessage("Özelleştirilmiş gider"),
    "customFeeWarning" : MessageLookupByLibrary.simpleMessage("Özelleştirilmiş giderleri yalnızca ne yaptığınızdan emin olduğunuzda kullanın !"),
    ...
    "decryptingWallet" : MessageLookupByLibrary.simpleMessage("Cüzdan deşifre ediliyor"),
    "delete" : MessageLookupByLibrary.simpleMessage("Sil"),
    "deleteConfirm" : MessageLookupByLibrary.simpleMessage("Devre dışı bırakmayı onayla"),
    "deleteSpan1" : MessageLookupByLibrary.simpleMessage("Kaldırmak istiyor musunuz"),
    "deleteSpan2" : MessageLookupByLibrary.simpleMessage("portföyünüzden mi? Eşleşmeyen tüm siparişler iptal edilecektir."),
    "deleteWallet" : MessageLookupByLibrary.simpleMessage("Cüzdanı Sil"),
    "deletingWallet" : MessageLookupByLibrary.simpleMessage("Cüzdan siliniyor.."),
    ...
    "dexIsNotAvailable" : MessageLookupByLibrary.simpleMessage("Bu koin DEX\'te yoktur"),
    "disclaimerAndTos" : MessageLookupByLibrary.simpleMessage("Yasal Uyarı ve Kullanım Şartları"),
    "done" : MessageLookupByLibrary.simpleMessage("Bitti"),
    "dontAskAgain" : MessageLookupByLibrary.simpleMessage("Tekrar sorma"),
    "dontWantPassword" : MessageLookupByLibrary.simpleMessage("Parola istemiyorum"),
    "duration" : MessageLookupByLibrary.simpleMessage("Süre"),
    "editContact" : MessageLookupByLibrary.simpleMessage("Kişiyi Düzenle"),
    "emptyCoin" : m18,
    "emptyExportPass" : MessageLookupByLibrary.simpleMessage("Parola şifreleme kısmı boş kalamaz"),
    "emptyImportPass" : MessageLookupByLibrary.simpleMessage("Parola kısmı boş kalamaz"),
    "emptyName" : MessageLookupByLibrary.simpleMessage("Kişi adı boş kalamaz"),
    "emptyWallet" : MessageLookupByLibrary.simpleMessage("Cüzdan adı boş kalamaz"),
    "enableTestCoins" : MessageLookupByLibrary.simpleMessage("Test Koinlerini Aktifleştir"),
    "enablingTooManyAssetsSpan1" : MessageLookupByLibrary.simpleMessage("Var"),
    "enablingTooManyAssetsSpan2" : MessageLookupByLibrary.simpleMessage("koininiz zaten ekli ve daha fazla etkinleştirmeye çalışıyorsunuz"),
    "enablingTooManyAssetsSpan3" : MessageLookupByLibrary.simpleMessage("Etkinleştirilebilir maksimum coin sayısı:"),
    "enablingTooManyAssetsSpan4" : MessageLookupByLibrary.simpleMessage("Yenilerini eklemeden önce lütfen eskilerinden birkaçını kaldırın."),
    "enablingTooManyAssetsTitle" : MessageLookupByLibrary.simpleMessage("Çok fazla sayıda koin etkinleştirmeye çalışıyorsunuz"),
    "encryptingWallet" : MessageLookupByLibrary.simpleMessage("Cüzdan şifreleniyor"),
    "englishLanguage" : MessageLookupByLibrary.simpleMessage("İngilizce"),
    "enter" : MessageLookupByLibrary.simpleMessage("Onayla"),
    "enterNewPinCode" : MessageLookupByLibrary.simpleMessage("Yeni PIN kodunu giriniz"),
    "enterOldPinCode" : MessageLookupByLibrary.simpleMessage("Eski PIN kodunu giriniz"),
    "enterPinCode" : MessageLookupByLibrary.simpleMessage("PIN Kodunuzu giriniz"),
    "enterSeedPhrase" : MessageLookupByLibrary.simpleMessage("Gizli kelimelerinizi giriniz"),
    "enterSellAmount" : MessageLookupByLibrary.simpleMessage("Önce satış miktarını girmelisiniz"),
    "enterpassword" : MessageLookupByLibrary.simpleMessage("Lütfen devam etmek için parolanızı giriniz"),
    "errorAmountBalance" : MessageLookupByLibrary.simpleMessage("Yeterli bakiye yok"),
    "errorNotAValidAddress" : MessageLookupByLibrary.simpleMessage("Geçerli bir adres değil"),
    "errorNotAValidAddressSegWit" : MessageLookupByLibrary.simpleMessage("Segwiit adresler henüz desteklenmemektir"),
    "errorNotEnoughGas" : m19,
    "errorTryAgain" : MessageLookupByLibrary.simpleMessage("Hata, lütfen tekrar deneyiniz"),
    "errorTryLater" : MessageLookupByLibrary.simpleMessage("Hata, lütfen daha sonra deneyiniz"),
    "errorValueEmpty" : MessageLookupByLibrary.simpleMessage("Değer çok yüksek ya da çok düşük"),
    "errorValueNotEmpty" : MessageLookupByLibrary.simpleMessage("Lütfen veri giriniz"),
    "estimateValue" : MessageLookupByLibrary.simpleMessage("Tahmini Toplam Değer"),
    "eulaParagraphe1" : m20,
    ...
    "exampleHintSeed" : MessageLookupByLibrary.simpleMessage("Örnek: build case level ..."),
    ...
    "exportDesc" : MessageLookupByLibrary.simpleMessage("Lütfen şifrelenmiş dosyaya aktarılacak öğeleri seçiniz."),
    "exportLink" : MessageLookupByLibrary.simpleMessage("Dışa Aktar"),
    "exportNotesTitle" : MessageLookupByLibrary.simpleMessage("Notlar"),
    "exportSuccessTitle" : MessageLookupByLibrary.simpleMessage("Öğeler dışa başarıyla aktarıldı."),
    ...
    "fakeBalanceAmt" : MessageLookupByLibrary.simpleMessage("Sahte bakiye tutarı:"),
    "faqTitle" : MessageLookupByLibrary.simpleMessage("Sıkça Sorulan Sorular"),
    ...
    "faucetTimedOut" : MessageLookupByLibrary.simpleMessage("Talep zaman aşımına uğradı"),
    "feedNewsTab" : MessageLookupByLibrary.simpleMessage("Haberler"),
    "feedNotFound" : MessageLookupByLibrary.simpleMessage("Bir şey yok"),
    "feedNotifTitle" : m35,
    "feedReadMore" : MessageLookupByLibrary.simpleMessage("Daha fazlası için.."),
    "feedTab" : MessageLookupByLibrary.simpleMessage("Bülten"),
    "feedTitle" : MessageLookupByLibrary.simpleMessage("Haber Bülteni"),
    "feedUnableToProceed" : MessageLookupByLibrary.simpleMessage("Haber güncellemeleri alınamıyor"),
    "feedUnableToUpdate" : MessageLookupByLibrary.simpleMessage("Haber güncellemeleri alınamıyor"),
    "feedUpToDate" : MessageLookupByLibrary.simpleMessage("Zaten güncel"),
    "feedUpdated" : MessageLookupByLibrary.simpleMessage("Haber bülteni güncellendi"),
    "feedback" : MessageLookupByLibrary.simpleMessage("Geri Bildirim Gönder"),
    "filtersAll" : MessageLookupByLibrary.simpleMessage("Hepsi"),
    "filtersButton" : MessageLookupByLibrary.simpleMessage("Filtre"),
    "filtersClearAll" : MessageLookupByLibrary.simpleMessage("Filtreleri temizle"),
    ...
    "generalPinNotActive" : MessageLookupByLibrary.simpleMessage("Normal PIN koruması aktif değil.\nKamuflajlı koruma modu aktif olmayacak.\nLütfen PIN korumasını aktifleştirin."),
    "getBackupPhrase" : MessageLookupByLibrary.simpleMessage("Önemli: Devam etmeden önce gizli kelimelerinizi güvenli bir şekilde yedekleyin !"),
    ...
    "hintConfirmPassword" : MessageLookupByLibrary.simpleMessage("Parolayı Onayla"),
    "hintCreatePassword" : MessageLookupByLibrary.simpleMessage("Parola Oluştur"),
    "hintCurrentPassword" : MessageLookupByLibrary.simpleMessage("Mevcut parola"),
    "hintEnterPassword" : MessageLookupByLibrary.simpleMessage("Parolanızı girin"),
    "hintEnterSeedPhrase" : MessageLookupByLibrary.simpleMessage("Gizli kelimelerinizi girin"),
    "hintNameYourWallet" : MessageLookupByLibrary.simpleMessage("Cüzdanınızı adlandırın"),
    ...
    "importDecryptError" : MessageLookupByLibrary.simpleMessage("Geçersiz şifre ya da hatalı dosya"),
    "importDesc" : MessageLookupByLibrary.simpleMessage("İçeri alınacaklar:"),
    "importFileNotFound" : MessageLookupByLibrary.simpleMessage("Dosya bulunamadı"),
    "importInvalidSwapData" : MessageLookupByLibrary.simpleMessage("Geçersiz takas bilgisi. Lütfen takas durumunu gösteren geçerli bir JSON dosyası ekleyin."),
    "importLink" : MessageLookupByLibrary.simpleMessage("İçeri Al"),
    "importLoadDesc" : MessageLookupByLibrary.simpleMessage("Lütfen içeri alınacak şifrelenmiş dosyayı seçin."),
    "importLoadSwapDesc" : MessageLookupByLibrary.simpleMessage("Lütfen içeri alınacak düz metin takas dosyasını seçin."),
    ...
    "importSingleSwapLink" : MessageLookupByLibrary.simpleMessage("Tek Takas İçeri Al"),
    "importSingleSwapTitle" : MessageLookupByLibrary.simpleMessage("Takas İçeri Al"),
    "importSomeItemsSkippedWarning" : MessageLookupByLibrary.simpleMessage("Bazı öğeler atlandı."),
    "importSuccessTitle" : MessageLookupByLibrary.simpleMessage("Öğeler başarıyla içeri alındı."),
    "importSwapFailed" : MessageLookupByLibrary.simpleMessage("Takas içeri alma başarısız oldu."),
    "importSwapJsonDecodingError" : MessageLookupByLibrary.simpleMessage("Json dosyası çözülürken hata oluştu."),
    "importTitle" : MessageLookupByLibrary.simpleMessage("İçeri Al"),
    "infoPasswordDialog" : MessageLookupByLibrary.simpleMessage("Güvenlı bir parola kullanın ve onu, uygulamayı kullandığınız cihazda saklamayın."),
    "infoTrade1" : MessageLookupByLibrary.simpleMessage("Takas talebi geri alınamaz ve bu nihai bir işlemdir !"),
    "infoTrade2" : MessageLookupByLibrary.simpleMessage("Bu işlem 60 dakikaya kadar sürebilir. Uygulamayı KAPATMAYINIZ !"),
    "infoWalletPassword" : MessageLookupByLibrary.simpleMessage("Cüzdanınızın güvenlik tehditlerine karşı şifrelenebilmesi için bir parola ayarlamalısınız."),
    "insufficientBalanceToPay" : m38,
    "insufficientText" : MessageLookupByLibrary.simpleMessage("Bu işlem için gereken en az hacim şu kadardır"),
    "insufficientTitle" : MessageLookupByLibrary.simpleMessage("Yetersiz hacim"),
    "internetRefreshButton" : MessageLookupByLibrary.simpleMessage("Yenile"),
    "internetRestored" : MessageLookupByLibrary.simpleMessage("İnternet Bağlantısı Yeniden Sağlandı"),
    "invalidSwap" : MessageLookupByLibrary.simpleMessage("Takasa devam edilemiyor"),
    ...
    "loadingOrderbook" : MessageLookupByLibrary.simpleMessage("Emir defteri yükleniyor..."),
    "lockScreen" : MessageLookupByLibrary.simpleMessage("Ekran kilitli"),
    "lockScreenAuth" : MessageLookupByLibrary.simpleMessage("Lütfen doğrulayın !"),
    "login" : MessageLookupByLibrary.simpleMessage("Giriş"),
    "logout" : MessageLookupByLibrary.simpleMessage("Çıkış"),
    "logoutOnExit" : MessageLookupByLibrary.simpleMessage("Kapanışta Çıkış Yap"),
    "logoutWarning" : MessageLookupByLibrary.simpleMessage("Çıkmak istediğinizden emin misiniz ?"),
    "logoutsettings" : MessageLookupByLibrary.simpleMessage("Çıkış Ayarları"),
    "longMinutes" : MessageLookupByLibrary.simpleMessage("dakikalar"),
    "makeAorder" : MessageLookupByLibrary.simpleMessage("bir emir girin"),
    "makerDetailsCancel" : MessageLookupByLibrary.simpleMessage("Emri iptal et"),
    "makerDetailsCreated" : MessageLookupByLibrary.simpleMessage("de oluşturuldu"),
    "makerDetailsFor" : MessageLookupByLibrary.simpleMessage("Almak"),
    "makerDetailsId" : MessageLookupByLibrary.simpleMessage("Emir ID"),
    "makerDetailsNoSwaps" : MessageLookupByLibrary.simpleMessage("Bu emirle herhangi bir takas başlamadı"),
    "makerDetailsPrice" : MessageLookupByLibrary.simpleMessage("Fiyat"),
    "makerDetailsSell" : MessageLookupByLibrary.simpleMessage("Sat"),
    "makerDetailsSwaps" : MessageLookupByLibrary.simpleMessage("Bu emirle takas başladı"),
    "makerDetailsTitle" : MessageLookupByLibrary.simpleMessage("Yapıcı Emri detayları"),
    ...
    "marketsNoAsks" : MessageLookupByLibrary.simpleMessage("Soru bulunamadı"),
    "marketsNoBids" : MessageLookupByLibrary.simpleMessage("Teklif bulunamadı"),
    "marketsOrderDetails" : MessageLookupByLibrary.simpleMessage("Emir Detayları"),
    "marketsOrderbook" : MessageLookupByLibrary.simpleMessage("EMİR DEFTERİ"),
    "marketsPrice" : MessageLookupByLibrary.simpleMessage("FİYAT"),
    "marketsSelectCoins" : MessageLookupByLibrary.simpleMessage("Lütfen koinleri seçiniz"),
    "marketsTab" : MessageLookupByLibrary.simpleMessage("Piyasalar"),
    "marketsTitle" : MessageLookupByLibrary.simpleMessage("PİYASALAR"),
    "matchExportPass" : MessageLookupByLibrary.simpleMessage("Parolalar uyuşmalı"),
    "matchingCamoChange" : MessageLookupByLibrary.simpleMessage("Değiştir"),
    "matchingCamoPinError" : MessageLookupByLibrary.simpleMessage("Normal PIN ve Kamuflajlı PIN kodlarınız aynı.\nKamuflajlı koruma modu aktif olmayacak.\nLütfen Kamuflajlı PIN kodunuzu değiştiriniz."),
    "matchingCamoTitle" : MessageLookupByLibrary.simpleMessage("Geçersiz PIN"),
    ...
    "mediaBrowseFeed" : MessageLookupByLibrary.simpleMessage("BÜLTENE GÖZAT"),
    "mediaBy" : MessageLookupByLibrary.simpleMessage("Yazar:"),
    "mediaNotSavedDescription" : MessageLookupByLibrary.simpleMessage("KAYDEDİLMİŞ BİR MAKALENİZ YOK"),
    "mediaSaved" : MessageLookupByLibrary.simpleMessage("KAYDEDİLDİ"),
    "merge" : MessageLookupByLibrary.simpleMessage("Birleştir"),
    "mergedValue" : MessageLookupByLibrary.simpleMessage("Birleştirilmiş değer:"),
    ...
    "minValueSell" : m43,
    "minVolumeInput" : m44,
    "minVolumeIsTDH" : MessageLookupByLibrary.simpleMessage("Satış miktarından düşük olmalı"),
    "minVolumeTitle" : MessageLookupByLibrary.simpleMessage("Gereken en düşük hacim"),
    "minVolumeToggle" : MessageLookupByLibrary.simpleMessage("hacim"),
    "minutes" : MessageLookupByLibrary.simpleMessage("dk"),
    "mobileDataWarning" : m45,
    "moreTab" : MessageLookupByLibrary.simpleMessage("Daha Fazla"),
    "multiActivateGas" : m46,
    "multiBaseAmtPlaceholder" : MessageLookupByLibrary.simpleMessage("Miktar"),
    ...
    "multiFiatDesc" : MessageLookupByLibrary.simpleMessage("Lütfen almak istediğiniz itibari para miktarını giriniz:"),
    "multiFiatFill" : MessageLookupByLibrary.simpleMessage("Otomatik doldur"),
    "multiFixErrors" : MessageLookupByLibrary.simpleMessage("Devam etmeden önce lütfen hataları gideriniz"),
    "multiInvalidAmt" : MessageLookupByLibrary.simpleMessage("Geçersiz miktar"),
    "multiInvalidSellAmt" : MessageLookupByLibrary.simpleMessage("Geçersiz satış miktarı"),
    "multiLowGas" : m48,
    "multiLowerThanFee" : m49,
    "multiMaxSellAmt" : MessageLookupByLibrary.simpleMessage("En fazla satış miktarı:"),
    "multiMinReceiveAmt" : MessageLookupByLibrary.simpleMessage("En az alım miktarı:"),
    "multiMinSellAmt" : MessageLookupByLibrary.simpleMessage("En satış miktarı:"),
    ...
    "multiTableAmt" : MessageLookupByLibrary.simpleMessage("Alınacak Miktar"),
    ...
    "noArticles" : MessageLookupByLibrary.simpleMessage("Şimdilik bir haber yok. Lütfen daha sonra tekrar deneyiniz."),
    "noCoinFound" : MessageLookupByLibrary.simpleMessage("Koin bulunamadı"),
    "noFunds" : MessageLookupByLibrary.simpleMessage("Fon yok"),
    "noFundsDetected" : MessageLookupByLibrary.simpleMessage("Fon mevcut değil. Lütfen para yatırın."),
    "noInternet" : MessageLookupByLibrary.simpleMessage("İnternet Bağlantısı Yok"),
    "noItemsToExport" : MessageLookupByLibrary.simpleMessage("Bir öğe seçilmedi"),
    "noItemsToImport" : MessageLookupByLibrary.simpleMessage("Bir öğe seçilmedi"),
    "noMatchingOrders" : MessageLookupByLibrary.simpleMessage("Eşleşen sipariş bulunamadı"),
    "noOrder" : m50,
    "noOrderAvailable" : MessageLookupByLibrary.simpleMessage("Emir oluşturmak için tıklayın"),
    "noOrders" : MessageLookupByLibrary.simpleMessage("Emir yok, lütfen ticarete gidin."),
    "noRewardYet" : MessageLookupByLibrary.simpleMessage("Alabileceğiniz bir ödül mevcut değil - 1 saat sonra yeniden deneyin."),
    "noRewards" : MessageLookupByLibrary.simpleMessage("Alınabilecek bir ödül yok"),
    "noSuchCoin" : MessageLookupByLibrary.simpleMessage("Böyle bir koin yok"),
    "noSwaps" : MessageLookupByLibrary.simpleMessage("Geçmiş yok."),
    "noTxs" : MessageLookupByLibrary.simpleMessage("İşlem Yok"),
    "nonNumericInput" : MessageLookupByLibrary.simpleMessage("Girilen değer rakam olmalı"),
    "notEnoughGas" : m51,
    "notEnoughtBalanceForFee" : MessageLookupByLibrary.simpleMessage("Takas masrafını karşılayacak bakiye bulunmuyor - daha küçük tutar deneyin"),
    "noteOnOrder" : MessageLookupByLibrary.simpleMessage("Eşleşmiş bir emir tekrardan iptal edilemez."),
    "notePlaceholder" : MessageLookupByLibrary.simpleMessage("Not Ekle"),
    "noteTitle" : MessageLookupByLibrary.simpleMessage("Not"),
    "nothingFound" : MessageLookupByLibrary.simpleMessage("Bir şey bulunamadı"),
    "notifSwapCompletedText" : m52,
    "notifSwapCompletedTitle" : MessageLookupByLibrary.simpleMessage("Takas tamamlandı"),
    "notifSwapFailedText" : m53,
    "notifSwapFailedTitle" : MessageLookupByLibrary.simpleMessage("Takas başarısız"),
    "notifSwapStartedText" : m54,
    "notifSwapStartedTitle" : MessageLookupByLibrary.simpleMessage("Yeni takas başladı"),
    "notifSwapStatusTitle" : MessageLookupByLibrary.simpleMessage("Takas durumu değişti"),
    "notifSwapTimeoutText" : m55,
    "notifSwapTimeoutTitle" : MessageLookupByLibrary.simpleMessage("Takas zaman aşımına uğradı"),
    "notifTxText" : m56,
    ...
    "oldLogsUsed" : MessageLookupByLibrary.simpleMessage("Boşluk kullanıldı"),
    "openMessage" : MessageLookupByLibrary.simpleMessage("Hata Mesajını Aç"),
    "orderCancel" : m58,
    "orderCreated" : MessageLookupByLibrary.simpleMessage("Emir oluşturuldu"),
    "orderCreatedInfo" : MessageLookupByLibrary.simpleMessage("Emir başarıyla oluşturuldu"),
    ...
    "orderDetailsExpedient" : m59,
    "orderDetailsExpensive" : m60,
    ...
    "orderDetailsSettings" : MessageLookupByLibrary.simpleMessage("Ayrıntıları tek dokunuşla açın ve Uzun dokunuşla sipariş et\'i seçin"),
    ...
    "orderTypeUnknown" : MessageLookupByLibrary.simpleMessage("Bilinmeyen Türde Emir"),
    ...
    "ordersTableAmount" : m62,
    "ordersTablePrice" : m63,
    "ordersTableTotal" : m64,
    "overwrite" : MessageLookupByLibrary.simpleMessage("Üstüne yaz"),
    "ownOrder" : MessageLookupByLibrary.simpleMessage("Bu sizin kendi emriniz !"),
    "paidFromBalance" : MessageLookupByLibrary.simpleMessage("Bakiyeden ödendi:"),
    "paidFromVolume" : MessageLookupByLibrary.simpleMessage("Alınan miktardan ödendi:"),
    "paidWith" : MessageLookupByLibrary.simpleMessage("Ödendi"),
    "passwordRequirement" : MessageLookupByLibrary.simpleMessage("Parolanız en az 12 karakterden oluşurken en az; bir büyük harf, bir küçük harf ve bir sembol içermelidir."),
    "paymentUriDetailsAccept" : MessageLookupByLibrary.simpleMessage("Öde"),
    "paymentUriDetailsAcceptQuestion" : MessageLookupByLibrary.simpleMessage("Bu işlemi kabul ediyor musunuz ?"),
    "paymentUriDetailsAddressSpan" : MessageLookupByLibrary.simpleMessage("Adrese"),
    "paymentUriDetailsAmountSpan" : MessageLookupByLibrary.simpleMessage("Miktar:"),
    "paymentUriDetailsCoinSpan" : MessageLookupByLibrary.simpleMessage("Koin:"),
    "paymentUriDetailsDeny" : MessageLookupByLibrary.simpleMessage("İptal"),
    "paymentUriDetailsTitle" : MessageLookupByLibrary.simpleMessage("Ödeme Talep Edildi"),
    "paymentUriInactiveCoin" : m65,
    "placeOrder" : MessageLookupByLibrary.simpleMessage("Emrinizi girin"),
    "pleaseAddCoin" : MessageLookupByLibrary.simpleMessage("Lütfen bir Koin Ekleyiniz"),
    "pleaseRestart" : MessageLookupByLibrary.simpleMessage("Yeniden denemek için lütfen uygulamayı yeniden başlatın ya da aşağıdaki düğmeye basınız."),
    "portfolio" : MessageLookupByLibrary.simpleMessage("Portföy"),
    "poweredOnKmd" : MessageLookupByLibrary.simpleMessage("Komodo tarafından geliştirilmiştir"),
    ...
    "protectionCtrlConfirmations" : MessageLookupByLibrary.simpleMessage("Onaylar"),
    "protectionCtrlCustom" : MessageLookupByLibrary.simpleMessage("Özel koruma seçenekleri kullan."),
    "protectionCtrlOff" : MessageLookupByLibrary.simpleMessage("Kapalı"),
    "protectionCtrlOn" : MessageLookupByLibrary.simpleMessage("Açık"),
    "protectionCtrlWarning" : MessageLookupByLibrary.simpleMessage("Dikkat ! Bu takas dPoW koruması altında değildir."),
    ...
    "question_7" : MessageLookupByLibrary.simpleMessage("Uygulamanın kullanılamadığı ülkeler var mı ?"),
    "question_8" : m69,
    "question_9" : m70,
    "receive" : MessageLookupByLibrary.simpleMessage("AL"),
    "receiveLower" : MessageLookupByLibrary.simpleMessage("Al"),
    "recommendSeedMessage" : MessageLookupByLibrary.simpleMessage("Çevrimdışı bir şekilde saklamanızı öneririz."),
    "remove" : MessageLookupByLibrary.simpleMessage("Devre dışı bırak"),
    "requestedTrade" : MessageLookupByLibrary.simpleMessage("Talep Edilen Alım Satım"),
    "reset" : MessageLookupByLibrary.simpleMessage("TEMİZLE"),
    "resetTitle" : MessageLookupByLibrary.simpleMessage("Geri Yükleme Formu"),
    "restoreWallet" : MessageLookupByLibrary.simpleMessage("GERİ YÜKLE"),
    "retryActivating" : MessageLookupByLibrary.simpleMessage("Tüm koinleri aktifleştirmeyi tekrar deniyor.."),
    "retryAll" : MessageLookupByLibrary.simpleMessage("Hepsini aktifleştirmeyi tekrar dene"),
    ...
    "rewardsError" : MessageLookupByLibrary.simpleMessage("Bir hata oluştu. Lütfen daha sonra tekrar deneyiniz."),
    "rewardsInProgressLong" : MessageLookupByLibrary.simpleMessage("İşlem sürmekte"),
    "rewardsInProgressShort" : MessageLookupByLibrary.simpleMessage("işleniyor"),
    "rewardsLowAmountLong" : MessageLookupByLibrary.simpleMessage("UTXO miktarı 10 KMD\'den az"),
    "rewardsLowAmountShort" : MessageLookupByLibrary.simpleMessage("<10 KMD"),
    "rewardsOneHourLong" : MessageLookupByLibrary.simpleMessage("Henüz 1 saat geçmedi"),
    "rewardsOneHourShort" : MessageLookupByLibrary.simpleMessage("<1 saat"),
    "rewardsPopupOk" : MessageLookupByLibrary.simpleMessage("Tamam"),
    "rewardsPopupTitle" : MessageLookupByLibrary.simpleMessage("Ödül durumu:"),
    "rewardsReadMore" : MessageLookupByLibrary.simpleMessage("KMD aktif kullanıcı ödülleri hakkında daha fazla bilgi edinin"),
    "rewardsReceive" : MessageLookupByLibrary.simpleMessage("Al"),
    "rewardsSuccess" : m71,
    "rewardsTableFiat" : MessageLookupByLibrary.simpleMessage("İtibari para"),
    "rewardsTableRewards" : MessageLookupByLibrary.simpleMessage("KMD Ödülleri"),
    ...
    "rewardsTableTitle" : MessageLookupByLibrary.simpleMessage("Ödül bilgisi:"),
    "rewardsTableUXTO" : MessageLookupByLibrary.simpleMessage("UTXO amt,\nKMD"),
    "rewardsTimeDays" : m72,
    "rewardsTimeHours" : m73,
    "rewardsTimeMin" : m74,
    "rewardsTitle" : MessageLookupByLibrary.simpleMessage("Ödül bilgisi"),
    "russianLanguage" : MessageLookupByLibrary.simpleMessage("Rusça"),
    "saveMerged" : MessageLookupByLibrary.simpleMessage("Birleştirilmiş kaydet"),
    "searchFilterCoin" : MessageLookupByLibrary.simpleMessage("Bir koin ara"),
    "searchFilterSubtitleAVX" : MessageLookupByLibrary.simpleMessage("Tüm AVAX tokenlerini seç"),
    "searchFilterSubtitleBEP" : MessageLookupByLibrary.simpleMessage("Tüm BEP tokenlerini seç"),
    "searchFilterSubtitleERC" : MessageLookupByLibrary.simpleMessage("Tüm ERC tokenlerini seç"),
    "searchFilterSubtitleETC" : MessageLookupByLibrary.simpleMessage("Tüm ETC tokenlerini seç"),
    "searchFilterSubtitleFTM" : MessageLookupByLibrary.simpleMessage("Tüm Fantom tokenlerini seç"),
    "searchFilterSubtitleHCO" : MessageLookupByLibrary.simpleMessage("Tüm HecoChain tokenlerini seç"),
    "searchFilterSubtitleHRC" : MessageLookupByLibrary.simpleMessage("Tüm Harmony tokenlerini seç"),
    "searchFilterSubtitleKRC" : MessageLookupByLibrary.simpleMessage("Tüm Kucoin tokenlerini seç"),
    "searchFilterSubtitleMVR" : MessageLookupByLibrary.simpleMessage("Tüm Moonriver tokenlerini seç"),
    "searchFilterSubtitlePLG" : MessageLookupByLibrary.simpleMessage("Tüm Polygon tokenlerini seç"),
    "searchFilterSubtitleQRC" : MessageLookupByLibrary.simpleMessage("Tüm QRC tokenlerini seç"),
    "searchFilterSubtitleSBCH" : MessageLookupByLibrary.simpleMessage("Tüm SmartBCH tokenlerini seç"),
    "searchFilterSubtitleSmartChain" : MessageLookupByLibrary.simpleMessage("Tüm SmartChain koinlerini seç"),
    "searchFilterSubtitleTestCoins" : MessageLookupByLibrary.simpleMessage("Tüm test koinlerini seç"),
    "searchFilterSubtitleUBQ" : MessageLookupByLibrary.simpleMessage("Tüm Ubiq koinlerini seç"),
    "searchFilterSubtitleutxo" : MessageLookupByLibrary.simpleMessage("Tüm UTXO koinlerini seç"),
    "searchForTicker" : MessageLookupByLibrary.simpleMessage("Kayan Yazı Ara"),
    "seconds" : MessageLookupByLibrary.simpleMessage("s"),
    "security" : MessageLookupByLibrary.simpleMessage("Güvenlik"),
    "seeOrders" : m75,
    "seeTxHistory" : MessageLookupByLibrary.simpleMessage("İşlem Geçmişini Görüntüle"),
    "seedPhrase" : MessageLookupByLibrary.simpleMessage("Gizli Kelimeler"),
    "seedPhraseTitle" : MessageLookupByLibrary.simpleMessage("Yeni Gizli Kelimeleriniz"),
    "selectCoin" : MessageLookupByLibrary.simpleMessage("Koin seçin"),
    "selectCoinInfo" : MessageLookupByLibrary.simpleMessage("Portföyünüze eklemek istediğiniz koinleri seçin."),
    "selectCoinTitle" : MessageLookupByLibrary.simpleMessage("Koin etkinleştir:"),
    "selectCoinToBuy" : MessageLookupByLibrary.simpleMessage("ALMAK istediğiniz koini seçin"),
    "selectCoinToSell" : MessageLookupByLibrary.simpleMessage("SATMAK istediğiniz koini seçin"),
    ...
    "selectPaymentMethod" : MessageLookupByLibrary.simpleMessage("Ödeme Yönteminizi Seçin"),
    "selectedOrder" : MessageLookupByLibrary.simpleMessage("Seçili emir:"),
    "sell" : MessageLookupByLibrary.simpleMessage("Sat"),
    "sellTestCoinWarning" : MessageLookupByLibrary.simpleMessage("Dikkat ! Test koinlerini herhangi bir değer olmadan satmak üzeresiniz"),
    "send" : MessageLookupByLibrary.simpleMessage("GÖNDER"),
    "setUpPassword" : MessageLookupByLibrary.simpleMessage("PAROLA BELİRLE"),
    "settingDialogSpan1" : MessageLookupByLibrary.simpleMessage("Silmek istediğine emin misin"),
    "settingDialogSpan2" : MessageLookupByLibrary.simpleMessage("cüzdan?"),
    "settingDialogSpan3" : MessageLookupByLibrary.simpleMessage("Eğer eminseniz"),
    "settingDialogSpan4" : MessageLookupByLibrary.simpleMessage("daha sonra cüzdanınızı kurtarabilmeniz için"),
    "settingDialogSpan5" : MessageLookupByLibrary.simpleMessage("gizli kelimelerinizi kaydedin."),
    ...
    "showMyOrders" : MessageLookupByLibrary.simpleMessage("EMİRLERİMİ GÖSTER"),
    "signInWithPassword" : MessageLookupByLibrary.simpleMessage("Parola ile giriş yap"),
    "signInWithSeedPhrase" : MessageLookupByLibrary.simpleMessage("Parolayı mı unuttunuz ? Gizli kelimelerinizle giriş yapın."),
    "simple" : MessageLookupByLibrary.simpleMessage("Basit"),
    "simpleTradeActivate" : MessageLookupByLibrary.simpleMessage("Aktifleştir"),
    "simpleTradeBuyHint" : m77,
    ...
    "swapGasActivate" : m85,
    "swapGasAmount" : m86,
    "swapGasAmountRequired" : m87,
    "swapOngoing" : MessageLookupByLibrary.simpleMessage("Takas devam ediyor"),
    ...
    "titleCreatePassword" : MessageLookupByLibrary.simpleMessage("PAROLA OLUŞTUR"),
    ...
    "tooManyAssetsEnabledSpan1" : MessageLookupByLibrary.simpleMessage("Var"),
    "tooManyAssetsEnabledSpan2" : MessageLookupByLibrary.simpleMessage("varlıklar etkinleştirildi. Atkif edilebilecek koin limiti:"),
    "tooManyAssetsEnabledSpan3" : MessageLookupByLibrary.simpleMessage("Yenilerini eklemeden önce lütfen eskilerden birkaçını kaldırın"),
    "tooManyAssetsEnabledTitle" : MessageLookupByLibrary.simpleMessage("Çok fazla koin aktifleştirildi"),
    "totalFees" : MessageLookupByLibrary.simpleMessage("Toplam Masraflar"),
    "trade" : MessageLookupByLibrary.simpleMessage("ALIM SATIM"),
    "tradeCompleted" : MessageLookupByLibrary.simpleMessage("TAKAS TAMAMLANDI !"),
    "tradeDetail" : MessageLookupByLibrary.simpleMessage("ALIM SATIM DETAYLARI"),
    "tradePreimageError" : MessageLookupByLibrary.simpleMessage("Alım satım işlem giderleri hesaplanamadı"),
    "tradingFee" : MessageLookupByLibrary.simpleMessage("Alım satım işlem gideri:"),
    "tradingMode" : MessageLookupByLibrary.simpleMessage("Alım Satım Modu:"),
    "tryRestarting" : MessageLookupByLibrary.simpleMessage("Bazı yeni koinler etkileştirilemedi ise uygulamayı tekrar başlatmayı deneyiniz."),
    ...
    "updatesChecking" : MessageLookupByLibrary.simpleMessage("Güncellemeler kontrol ediliyor.."),
    "updatesCurrentVersion" : m90,
    "updatesNotifAvailable" : MessageLookupByLibrary.simpleMessage("Yeni sürüm mevcut. Lütfen güncelleyiniz."),
    "updatesNotifAvailableVersion" : m91,
    "updatesNotifTitle" : MessageLookupByLibrary.simpleMessage("Güncelleme mevcut"),
    "updatesSkip" : MessageLookupByLibrary.simpleMessage("Şimdilik atla"),
    "updatesTitle" : m92,
    "updatesUpToDate" : MessageLookupByLibrary.simpleMessage("Son sürüm yüklü"),
    "updatesUpdate" : MessageLookupByLibrary.simpleMessage("Güncelle"),
    "uriInsufficientBalanceSpan1" : MessageLookupByLibrary.simpleMessage("Taranan ödeme talebi için"),
    "uriInsufficientBalanceSpan2" : MessageLookupByLibrary.simpleMessage("yeterli bakiye yok."),
    "uriInsufficientBalanceTitle" : MessageLookupByLibrary.simpleMessage("Yetersiz bakiye"),
    "value" : MessageLookupByLibrary.simpleMessage("Değer:"),
    "version" : MessageLookupByLibrary.simpleMessage("sürüm"),
    "viewInExplorerButton" : MessageLookupByLibrary.simpleMessage("Tarayıcı"),
    "viewSeedAndKeys" : MessageLookupByLibrary.simpleMessage("Gizli ve Özel Kelimeler"),
    "volumes" : MessageLookupByLibrary.simpleMessage("Hacim"),
    "walletInUse" : MessageLookupByLibrary.simpleMessage("Bu cüzdan adı zaten kullanımda"),
    "walletMaxChar" : MessageLookupByLibrary.simpleMessage("Cüzdan adı en fazla 40 karakter içerebilir"),
    ...
    "warningShareLogs" : MessageLookupByLibrary.simpleMessage("Dikkat ! Bu kayıt defteri özel durumlarda, başarısız takaslardan kalan koinlerin harcanmasına sebep olabilecek hassas bilgiler içerebilir."),
    "weFailedTo" : m93,
    "weFailedToActivate" : m94,
    "welcomeInfo" : m95,
    "welcomeLetSetUp" : MessageLookupByLibrary.simpleMessage("HADİ AYARLAYALIM !"),
    "welcomeTitle" : MessageLookupByLibrary.simpleMessage("HOŞ GELDİNİZ"),
    "welcomeWallet" : MessageLookupByLibrary.simpleMessage("cüzdan"),
    "willBeRedirected" : MessageLookupByLibrary.simpleMessage("Tamamlandığında portföy ekranına yönlendirileceksiniz."),
    "withdraw" : MessageLookupByLibrary.simpleMessage("Çek"),
    "withdrawCameraAccessText" : m96,
    "withdrawCameraAccessTitle" : MessageLookupByLibrary.simpleMessage("Erişim Engellendi"),
    "withdrawConfirm" : MessageLookupByLibrary.simpleMessage("Çekimi Onayla"),
    "withdrawConfirmError" : MessageLookupByLibrary.simpleMessage("Bir hata oluştu. Lütfen daha sonra tekrar deneyiniz."),
    "withdrawValue" : m97,
    "wrongCoinSpan1" : MessageLookupByLibrary.simpleMessage("QR kodlu bir ödemeyi"),
    "wrongCoinSpan2" : MessageLookupByLibrary.simpleMessage("taramaya çalışıyorsunuz"),
    "wrongCoinSpan3" : MessageLookupByLibrary.simpleMessage("fakat çekim ekranındasınız"),
