...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m2(abbr) => "No puede enviar fondos a la dirección ${abbr} porque ${abbr} no está activado. Por favor, vaya a la cartera.";

  static m3(appName) => "¡No! ${appName} no tiene custodia. Nunca almacenamos datos confidenciales, incluidas sus claves privadas, frases iniciales o PIN. Estos datos solo se almacenan en el dispositivo del usuario y nunca lo abandonan. Usted tiene el control total de sus activos.";

  static m4(appName) => "${appName} está disponible para dispositivos móviles en Android y iPhone, y para computadoras de escritorio en los sistemas operativos Windows, Mac y Linux.";

  static m5(appName) => "Por lo general, otros DEX solo le permiten intercambiar monedas digitales que se basan en una sola red blockchain y solo permiten realizar un solo pedido con los mismos fondos.\n\n${appName} le permite intercambiar de forma nativa en blockchains diferentes. También puede realizar varios pedidos con los mismos fondos. Por ejemplo, puede vender 0.1 BTC por KMD, QTUM o VRSC: el primer pedido que se ejecuta automáticamente cancela todos los demás pedidos.";

  static m6(appName) => "Varios factores determinan el tiempo de procesamiento de cada intercambio. El tiempo de procesar una transacción depende de cada red (Bitcoin suele ser la más lenta). Además, el usuario puede personalizar las preferencias de seguridad. Por ejemplo, puede pedirle a ${appName} que considere una transacción KMD procesada despues de solo 3 confirmaciones, lo que hace que el tiempo de intercambio sea más corto en comparación con la espera de una <a href=\"https://komodoplatform.com/security-delayed-proof-of-work-dpow/\">notarization</a>.";

  static m7(appName) => "Hay dos categorías de tarifas a tener en cuenta al intercambiar en ${appName}.\n\n1. ${appName} cobra aproximadamente un 0,13 % (1/777 del volumen de la transacción, pero no menos de 0,0001) como tarifa de transacción para las órdenes del comprador, y las órdenes del vendedor no tienen tarifas.\n\n2. Tanto los vendedores como los compradores deberán pagar tarifas de red normales a las blockchains involucradas al realizar transacciones de intercambio atómico.\n\nLas tarifas de red pueden variar mucho según el par de monedas seleccionadas.";

  static m8(name, link, appName, appCompanyShort) => "¡Sí! ${appName} ofrece soporte a través de <a href=\"${link}\">${appCompanyShort} ${name}</a>. ¡El equipo y la comunidad siempre están dispuestos a ayudar!";

  static m9(appName) => "¡No! ${appName} es completamente descentralizado. No es posible limitar el acceso de los usuarios por parte de ningún tercero.";

  static m10(appName, appCompanyShort) => "${appName} está desarrollado por el equipo de ${appCompanyShort}. ${appCompanyShort} es uno de los proyectos de blockchain más consolidados que trabaja en soluciones innovadoras como intercambios decentralizados, seguridad de blockchains y una arquitectura multicadena interoperable.";

  static m11(appName) => "¡Absolutamente! Puede leer nuestra <a href=\"https://developers.atomicdex.io/\">documentación para desarrolladores</a> para obtener más detalles o comunicarse con nosotros si tiene alguna consulta sobre su asociación. ¿Tiene una pregunta técnica específica? ¡La comunidad de desarrolladores de ${appName} siempre está lista para ayudar!";

  static m12(batteryLevelCritical) => "La carga de su batería es crítica (${batteryLevelCritical}%) para realizar un intercambio de manera segura. Póngalo a cargar y vuelva a intentarlo.";
  ...
  static m35(coin) => "Enviando solicitud a la llave ${coin}...";
  ...
  static m42(coinName, number) => "La cantidad mínima para comprar es ${number}${coinName}";

  static m43(buyCoin, buyAmount, sellCoin, sellAmount) => "El monto mínimo del pedido es ${buyAmount}${buyCoin}\n(${sellAmount}${sellCoin})";
  ...
  static m60(delta) => "Expediente: CEX +${delta}%";

  static m61(delta) => "Caro: CEX ${delta}%";

  static m62(fill) => "${fill}% llenado";
  ...
  static m66(abbr) => "${abbr} no está activo. Activa e inténtalo de nuevo..";

  static m67(appName) => "¿En qué dispositivos puedo usar ${appName}?";

  static m68(appName) => "¿En qué se diferencia el comercio en ${appName} del comercio en otros DEX?";

  static m69(appName) => "¿Cómo se calculan las tarifas de ${appName}?";

  static m70(appName) => "¿Quién está detrás de ${appName}?";
  ...
  static m79(maxCoins) => "El número máximo de monedas activas es ${maxCoins}. Por favor, desactive algunos.";

  static m80(coin) => "${coin} no está activo!";
  ...
  static m86(coin) => "Activa ${coin} y recarga el saldo primero";
  ...
  static Map<String, Function> _notInlinedMessages(_) => <String, Function> {
    ...
    "exampleHintSeed" : MessageLookupByLibrary.simpleMessage("Ejemplo: compilar nivel de caso..."),
    ...
    "traditionalChinese" : MessageLookupByLibrary.simpleMessage("tradicional"),
